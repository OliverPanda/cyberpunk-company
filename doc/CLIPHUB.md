...
ClipHub is the public registry where people share, discover, and download Cyberpunk Company company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Cyberpunk Company what a package registry is to a programming language. Cyberpunk Company already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Cyberpunk Company — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Cyberpunk Company instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Cyberpunk Company spec. It contains:
...
**Install (fresh start):**
```
cyberpunk-company install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Cyberpunk Company instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Cyberpunk Company, export your company as a template, then publish:

```
cyberpunk-company export --template my-company
cyberpunk-company publish cliphub my-company
```
...
```
cyberpunk-company cliphub sync
```
...
ClipHub is a **separate service** from Cyberpunk Company itself. Cyberpunk Company is self-hosted; ClipHub is a hosted registry that Cyberpunk Company instances talk to.
...
| **Cyberpunk Company CLI** | `cyberpunk-company install`, `cyberpunk-company publish`, `cyberpunk-company cliphub sync` — built into Cyberpunk Company |
| **Cyberpunk Company UI** | "Browse ClipHub" panel in the Cyberpunk Company web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Cyberpunk Company) |
| Backend | TypeScript + Hono (consistent with Cyberpunk Company) |
| Database | PostgreSQL |
...
4. Run `cyberpunk-company install cliphub:acme/lean-saas-shop`
5. Cyberpunk Company creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Cyberpunk Company until it works well
2. Export: `cyberpunk-company export --template my-agency`
3. Publish: `cyberpunk-company publish cliphub my-agency`
...
3. Install just that agent: `cyberpunk-company install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Cyberpunk Company

ClipHub is **not required** to use Cyberpunk Company. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Cyberpunk Company what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `cyberpunk-company install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Cyberpunk Company web UI
- [ ] `cyberpunk-company cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Cyberpunk Company's own philosophy)
