...
For `claude_local` and `codex_local`, Cyberpunk Company assumes the CLI is already installed and authenticated on the host machine.
...
Cyberpunk Company stores resumable session state per `(agent, taskKey, adapterType)`.
