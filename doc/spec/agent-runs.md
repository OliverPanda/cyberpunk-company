...
This spec defines how Cyberpunk Company actually runs agents while staying runtime-agnostic.
...
1. Cyberpunk Company is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Cyberpunk Company manages lifecycle, and on exit Cyberpunk Company parses JSON output and updates state.
...
```json
{
  "runLogStore": {
    ...
    "bucket": "cyberpunk-company-run-logs",
    "prefix": "runs/",
    "compress": true,
...
- `run.startedAt`
- `heartbeat.reason`
- `cyberpunk-company.skill` (shared Cyberpunk Company skill text block)
...
2. Preferred transport is env vars (`CYBERPUNK_*`) injected at runtime.
