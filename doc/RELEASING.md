# Releasing Cyberpunk Company

Maintainer runbook for shipping Cyberpunk Company across npm, GitHub, and the website-facing changelog surface.
...
Cyberpunk Company uses calendar versions that still fit semver syntax:
...
2. **npm** — `cyberpunk-company` and public workspace packages are published
...
```bash
npx cyberpunk-company@canary onboard
# or
npx cyberpunk-company@canary onboard --data-dir "$(mktemp -d /tmp/cyberpunk-company-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/cyberpunk-company/cyberpunk-company/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Cyberpunk Company. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
CYBERPUNK COMPANYAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
CYBERPUNK COMPANYAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary CYBERPUNK COMPANYAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable CYBERPUNK COMPANYAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- `npx cyberpunk-company@canary onboard` installs
