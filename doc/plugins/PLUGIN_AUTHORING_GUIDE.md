...
This guide describes the current, implemented way to create a Cyberpunk Company plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Cyberpunk Company app.
...
```bash
pnpm --filter @cyberpunk-company/create-cyberpunk-company-plugin build
node packages/plugins/create-cyberpunk-company-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Cyberpunk Company repo:

```bash
pnpm --filter @cyberpunk-company/create-cyberpunk-company-plugin build
node packages/plugins/create-cyberpunk-company-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/cyberpunk-company/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@cyberpunk-company/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@cyberpunk-company/plugin-sdk` from the local Cyberpunk Company checkout into a `.cyberpunk-company-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Cyberpunk Company from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@cyberpunk-company/plugin-sdk/ui`
