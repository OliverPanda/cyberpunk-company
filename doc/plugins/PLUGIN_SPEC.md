# Cyberpunk Company Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Cyberpunk Company's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Cyberpunk Company app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Cyberpunk Company HTTP APIs directly.
...
Cyberpunk Company plugin design is based on the following assumptions:

1. Cyberpunk Company is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Cyberpunk Company business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Cyberpunk Company core.
...
2. Let plugins add major capabilities without editing Cyberpunk Company core.
...
4. Depend on project-local plugin folders such as `.cyberpunk-company/plugins`.
...
The single Cyberpunk Company deployment an operator installs and controls.

### 5.2 Company

A first-class Cyberpunk Company business object inside the instance.
...
A trusted in-process extension loaded directly by Cyberpunk Company core.
...
An installable instance-wide extension package loaded through the Cyberpunk Company plugin runtime.
...
Cyberpunk Company has two extension classes.
...
Cyberpunk Company already has a concrete workspace model:
...
Plugins live under the Cyberpunk Company instance directory.

Suggested layout:

- `~/.cyberpunk-company/instances/default/plugins/package.json`
- `~/.cyberpunk-company/instances/default/plugins/node_modules/`
- `~/.cyberpunk-company/instances/default/plugins/.cache/`
- `~/.cyberpunk-company/instances/default/data/plugins/<plugin-id>/`
...
Cyberpunk Company should add CLI commands:

- `pnpm cyberpunk-company plugin list`
- `pnpm cyberpunk-company plugin install <package[@version]>`
- `pnpm cyberpunk-company plugin uninstall <plugin-id>`
- `pnpm cyberpunk-company plugin upgrade <plugin-id> [version]`
- `pnpm cyberpunk-company plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@cyberpunk-company/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface Cyberpunk CompanyPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumCyberpunk CompanyVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Cyberpunk Company agents can use during runs.
...
- Cyberpunk Company server starts one worker process per installed plugin
...
Receives one typed Cyberpunk Company domain event.
...
export function definePlugin(definition: PluginDefinition): Cyberpunk CompanyPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: Cyberpunk CompanyPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@cyberpunk-company/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@cyberpunk-company/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@cyberpunk-company/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@cyberpunk-company/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@cyberpunk-company/plugin-sdk/ui`
...
The `@cyberpunk-company/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Cyberpunk Company secret provider system rather than a plain text input
...
1. Core Cyberpunk Company data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Cyberpunk Company objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Cyberpunk Company product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Cyberpunk Company later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Cyberpunk Company secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm cyberpunk-company plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Cyberpunk Company server. This is a normative requirement, not optional.
...
### 27.1 `@cyberpunk-company/plugin-test-harness`
...
```ts
import { createTestHarness } from "@cyberpunk-company/plugin-test-harness";
...
For developing a plugin against a running Cyberpunk Company instance:

- The operator installs the plugin from a local path: `pnpm cyberpunk-company plugin install ./path/to/plugin`
...
- `@cyberpunk-company/plugin-workspace-files`
- `@cyberpunk-company/plugin-terminal`
- `@cyberpunk-company/plugin-git`
- `@cyberpunk-company/plugin-linear`
- `@cyberpunk-company/plugin-github-issues`
- `@cyberpunk-company/plugin-grafana`
- `@cyberpunk-company/plugin-runtime-processes`
- `@cyberpunk-company/plugin-stripe`
...
- `@cyberpunk-company/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@cyberpunk-company/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@cyberpunk-company/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@cyberpunk-company/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@cyberpunk-company/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@cyberpunk-company/plugin-sdk/ui`
...
- `@cyberpunk-company/plugin-test-harness`
- `create-cyberpunk-company-plugin` starter template
...
Cyberpunk Company should not implement a generic in-process hook bag modeled directly after local coding tools.

Cyberpunk Company should implement:
...
That is the complete target design for the Cyberpunk Company plugin system.
