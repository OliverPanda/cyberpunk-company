...
Cyberpunk Company V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Cyberpunk Company-shaped design.

Assumption for this document: Cyberpunk Company is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Cyberpunk Company objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Cyberpunk Company.

The main conclusion is:

- Cyberpunk Company should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Cyberpunk Company should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Cyberpunk Company should use multiple extension classes instead of one generic plugin bag:
...
If Cyberpunk Company does this well, the examples you listed become straightforward:
...
Relevant Cyberpunk Company files reviewed for current extension seams:
...
It is too dangerous for Cyberpunk Company core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Cyberpunk Company — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Cyberpunk Company should absolutely do this.
...
Cyberpunk Company should copy this discipline.
...
Cyberpunk Company should adopt this style for plugin actions, automations, and UI schemas.
...
Cyberpunk Company can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Cyberpunk Company too.

## What Cyberpunk Company Should Not Copy Directly
...
Cyberpunk Company is a control plane for an operator-managed instance with company objects.
...
Cyberpunk Company is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Cyberpunk Company should not auto-load arbitrary code from a workspace repo like `.cyberpunk-company/plugins` or project directories.
...
For Cyberpunk Company, equivalent hooks into:
...
Cyberpunk Company should not allow plugins to silently replace:
...
For Cyberpunk Company it would be risky because it combines:
...
Cyberpunk Company should require an explicit operator install step.

## Why Cyberpunk Company Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Cyberpunk Company |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Cyberpunk Company instance |
...
That means Cyberpunk Company should borrow the good ideas from `opencode` but use a stricter architecture.

## Cyberpunk Company Already Has Useful Pre-Plugin Seams

Cyberpunk Company has several extension-like seams already:
...
This is good news.
Cyberpunk Company does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Cyberpunk Company Plugin Model
...
Cyberpunk Company already has this pattern implicitly:
...
- `plugin` means integration code that talks to Cyberpunk Company through a typed plugin protocol and capability model
...
- invoke explicit Cyberpunk Company actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@cyberpunk-company/plugin-sdk/ui`) with design tokens and common components.
...
Cyberpunk Company should treat plugin installation as a global instance-level action.

Examples:

- install `@cyberpunk-company/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Cyberpunk Company objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Cyberpunk Company already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Cyberpunk Company too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@cyberpunk-company/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@cyberpunk-company/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Cyberpunk Company instance.

A `create-cyberpunk-company-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Cyberpunk Company achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Cyberpunk Company server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Cyberpunk Company's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@cyberpunk-company/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@cyberpunk-company/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Cyberpunk Company

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@cyberpunk-company/plugin-sdk";

export default definePlugin({
  id: "@cyberpunk-company/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Cyberpunk Company entities
    });
    ...
    ctx.events.on("plugin.@cyberpunk-company/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@cyberpunk-company/plugin-sdk/ui";
...
Any Cyberpunk Company plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Cyberpunk Company objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@cyberpunk-company/plugin-linear`)
...
- reuse core tables when the data is clearly part of Cyberpunk Company itself
...
If a concept is becoming part of Cyberpunk Company's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Cyberpunk Company itself
...
### 3. Scope plugin data to Cyberpunk Company objects before adding custom schemas

A lot of plugin data naturally hangs off existing Cyberpunk Company objects:
...
If Cyberpunk Company eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@cyberpunk-company/plugin-workspace-files`
...
Package idea: `@cyberpunk-company/plugin-terminal`
...
Package idea: `@cyberpunk-company/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Cyberpunk Company.
- Board reviews the diff after a run without leaving Cyberpunk Company.
...
- optional `events.emit` (e.g. `plugin.@cyberpunk-company/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@cyberpunk-company/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@cyberpunk-company/plugin-linear`

This plugin syncs Cyberpunk Company work with Linear. It is useful for:

- importing backlog from Linear
- linking Cyberpunk Company issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Cyberpunk Company and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Cyberpunk Company.
- Cyberpunk Company issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@cyberpunk-company/plugin-github-issues`

This plugin syncs Cyberpunk Company issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Cyberpunk Company issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Cyberpunk Company task governance
...
- Plugin settings:
  ...
  - whether new Cyberpunk Company issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Cyberpunk Company issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Cyberpunk Company.
- GitHub webhooks update status/comment state in Cyberpunk Company.
- A PR is linked back to the Cyberpunk Company issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@cyberpunk-company/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@cyberpunk-company/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Cyberpunk Company. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Cyberpunk Company issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Cyberpunk Company plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Cyberpunk Company dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Cyberpunk Company issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Cyberpunk Company core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@cyberpunk-company/plugin-runtime-processes`
...
Package idea: `@cyberpunk-company/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Cyberpunk Company. It is useful for:
...
- Failed payment spikes or churn events can generate Cyberpunk Company issues for follow-up.
...
- Stripe data should stay additive to Cyberpunk Company core
...
- plugin UI bundle loading, host bridge, `@cyberpunk-company/plugin-sdk/ui`
...
- `@cyberpunk-company/plugin-test-harness` and `create-cyberpunk-company-plugin` starter template
...
Cyberpunk Company should not implement "an OpenCode-style generic in-process hook system."
Cyberpunk Company should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Cyberpunk Company
...
8. Ship `@cyberpunk-company/plugin-test-harness` and `create-cyberpunk-company-plugin` starter template.
