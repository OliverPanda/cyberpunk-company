...
This document summarizes the memory systems referenced in task `PAP-530` and extracts the design patterns that matter for Cyberpunk Company.

## What Cyberpunk Company Needs From This Survey

Cyberpunk Company is not trying to become a single opinionated memory engine. The more useful target is a control-plane memory surface that:

- stays company-scoped
- lets each company choose a default memory provider
- lets specific agents override that default
- keeps provenance back to Cyberpunk Company runs, issues, comments, and documents
...
The question is not "which memory project wins?" The question is "what is the smallest Cyberpunk Company contract that can sit above several very different memory systems without flattening away the useful differences?"
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because Cyberpunk Company is local-first today and needs at least one zero-config path.

## Per-Project Notes

| Project | Shape | Notable API / model | Strong fit for Cyberpunk Company | Main mismatch |
|---|---|---|---|---|
...
| [mem0](https://github.com/mem0ai/mem0) | hosted + OSS SDK | `add`, `search`, `getAll`, `get`, `update`, `delete`, `deleteAll`; entity partitioning via `user_id`, `agent_id`, `run_id`, `app_id` | closest to a clean provider API with identities and metadata filters | provider owns extraction heavily; Cyberpunk Company should not assume every backend behaves like mem0 |
| [MemOS](https://github.com/MemTensor/MemOS) | memory OS / framework | unified add-retrieve-edit-delete, memory cubes, multimodal memory, tool memory, async scheduler, feedback/correction | strong source for optional capabilities beyond plain search | much broader than the minimal contract Cyberpunk Company should standardize first |
...
If Cyberpunk Company does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why Cyberpunk Company needs a layered contract instead of a single hard-coded engine.
...
Cyberpunk Company should support both:

- provider-managed extraction
- Cyberpunk Company-managed extraction with provider-managed storage / retrieval
...
Cyberpunk Company should not require a single storage shape. It should require normalized references back to Cyberpunk Company entities.
...
Cyberpunk Company should make plain search the minimum contract and richer outputs optional capabilities.
...
Cyberpunk Company needs both direct request/response operations and background maintenance hooks.

## Cyberpunk Company-Specific Takeaways

### Cyberpunk Company should own these concerns

- binding a provider to a company and optionally overriding it per agent
- mapping Cyberpunk Company entities into provider scopes
- provenance back to issue comments, documents, runs, and activity
- cost / token / latency reporting for memory work
- browse and inspect surfaces in the Cyberpunk Company UI
...
Cyberpunk Company does not need to standardize every feature from every provider. It needs:
...
Cyberpunk Company should adopt a two-layer memory model:

1. `Memory binding + control plane layer`
   Cyberpunk Company decides which provider key is in effect for a company, agent, or project, and it logs every memory operation with provenance and usage.

2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns Cyberpunk Company memory requests into provider-specific calls.
...
without forcing Cyberpunk Company itself to become a monolithic memory engine.
