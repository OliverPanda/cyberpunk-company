...
Cyberpunk Company needs to support two very different but equally valid ways of working:
...
Today, Cyberpunk Company already has the beginnings of this model:
...
- making Cyberpunk Company feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Cyberpunk Company users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Cyberpunk Company deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Cyberpunk Company into a full code review product
...
This object must be recorded explicitly so that Cyberpunk Company can:
...
Cyberpunk Company should treat PRs as a type of work product linked back to:
...
Cyberpunk Company already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Cyberpunk Company must be a first-class deployment mode

Cyberpunk Company cannot assume that it is running on the same machine as the code.

In cloud deployments, Cyberpunk Company may:
...
- `Runtime service`: a process or service Cyberpunk Company owns or tracks for a workspace
...
It also lets cloud-hosted Cyberpunk Company deployments point at codebases and remotes without pretending the Cyberpunk Company host has direct filesystem access.
...
This lets Cyberpunk Company support:
...
Without an explicit `execution workspace` record, Cyberpunk Company has nowhere stable to attach:
...
Cyberpunk Company needs a single place to show:
...
- Cyberpunk Company-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Cyberpunk Company owning the execution host
...
## 1. Cyberpunk Company host and execution host must be decoupled

Cyberpunk Company may run:
...
without the Cyberpunk Company host itself running local git or local preview processes.
...
Cyberpunk Company should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Cyberpunk Company.
...
Cyberpunk Company should keep workspace automation centered on a small set of hooks:
...
Cyberpunk Company should provide the equivalent visibility in the project `Code` surface:
...
Cyberpunk Company should treat execution workspaces as the natural home for:
...
Cyberpunk Company should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Cyberpunk Company should continue treating cleanup policy as part of the core workspace model:
...
Cyberpunk Company should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Cyberpunk Company host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Cyberpunk Company should borrow Worktrunk's good execution patterns while keeping the broader Cyberpunk Company model:
...
This is how Cyberpunk Company supports:
...
That keeps Cyberpunk Company focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Cyberpunk Company deployments remain first-class

That is a better fit for Cyberpunk Company than either extreme:
