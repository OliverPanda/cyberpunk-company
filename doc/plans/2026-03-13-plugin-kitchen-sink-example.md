...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Cyberpunk Company plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Cyberpunk Company.
...
- package: `@cyberpunk-company/plugin-kitchen-sink-example`
- plugin id: `cyberpunk-company.kitchen-sink-example` or `cyberpunk-company-kitchen-sink-example`

Recommendation: use `cyberpunk-company-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Cyberpunk Company event such as `issue.created`
...
Recommendation: default to a Cyberpunk Company-local endpoint or a stable public echo endpoint to avoid flaky docs.
