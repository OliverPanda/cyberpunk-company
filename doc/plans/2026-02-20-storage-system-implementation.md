...
Add a single storage subsystem for Cyberpunk Company that supports:
...
- API serves attachment bytes through authenticated Cyberpunk Company endpoints.
...
- Default local path is under instance root: `~/.cyberpunk-company/instances/<instanceId>/data/storage`.
...
- `cyberpunk-company onboard` writes a valid `storage` config block by default.
- `cyberpunk-company configure --section storage` can switch between local and s3 modes.
...
- `cyberpunk-company doctor` reports actionable storage status.
