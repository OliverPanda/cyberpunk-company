...
This document defines the V1 design for a Cyberpunk Company adapter that integrates with
...
4. synthesized stdout events for Cyberpunk Company UI/CLI
...
3. **Agent callback auth to Cyberpunk Company** uses a bootstrap exchange flow (no long-lived Cyberpunk Company key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Cyberpunk Company endpoints, not full SKILL.md prompt injection.
...
## Cyberpunk Company Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `CYBERPUNK_API_KEY`.
...
Goal: avoid putting long-lived Cyberpunk Company credentials in prompt text.

Flow:

1. Before launch/follow-up, Cyberpunk Company mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Cyberpunk Company validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Cyberpunk Company API calls.
...
1. Prompt includes a compact instruction to fetch skills from Cyberpunk Company.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/cyberpunk-company`
   - `GET /api/skills/cyberpunk-company-create-agent` when needed
...
4. Route by Cursor `agentId` to active Cyberpunk Company run context.
...
- poll interval
- timeout/grace
- cyberpunk-company public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from Cyberpunk Company skill endpoints |
| Cyberpunk Company auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Cyberpunk Company context.
3. Add richer PR metadata surfacing in Cyberpunk Company UI.
