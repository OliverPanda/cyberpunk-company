...
- New config/env settings:
  - `CYBERPUNK_AGENT_JWT_SECRET`
  - `CYBERPUNK_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `CYBERPUNK_AGENT_JWT_ISSUER` (default: `cyberpunk-company`)
  - `CYBERPUNK_AGENT_JWT_AUDIENCE` (default: `cyberpunk-company-api`)
...
1. In:
   ...
   inject `CYBERPUNK_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `CYBERPUNK_API_KEY`, do not overwrite it.
- Continue injecting:
  - `CYBERPUNK_AGENT_ID`
...
- Local adapters authenticate without manual `CYBERPUNK_API_KEY` config.
