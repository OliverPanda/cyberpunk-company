# Cyberpunk Company Module System
...
Cyberpunk Company's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Cyberpunk Company's API, UI, and data model | Yes |
...
modules/
  observability/
    cyberpunk-company.module.json     # manifest (required)
    src/
...
### Manifest (`cyberpunk-company.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@cyberpunk-company/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@cyberpunk-company/core";
...
```
1. Scan modules/ directory for cyberpunk-company.module.json manifests
...
```jsonc
// cyberpunk-company.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@cyberpunk-company/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@cyberpunk-company/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:cyberpunk-company/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.cyberpunk-company.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm cyberpunk-company store list                    # browse available modules and templates
pnpm cyberpunk-company store install <module-id>     # install a module
pnpm cyberpunk-company store import <template-id>    # import a company template
pnpm cyberpunk-company store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Cyberpunk Company instance | `server:started` |
...
Add to `@cyberpunk-company/server`:
...
4. **Module config** — `cyberpunk-company.config.json` with per-module config, env var interpolation
...
10. **`@cyberpunk-company/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
