...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/cyberpunk-company-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `CYBERPUNK_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `CYBERPUNK_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Cyberpunk Company may create host-local worktrees, processes, and paths
- `cloud`
  - Cyberpunk Company should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `cyberpunk-company | github | vercel | s3 | custom`
...
- use `CYBERPUNK_EXECUTION_TOPOLOGY` as a defaulting guardrail
