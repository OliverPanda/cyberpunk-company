...
Agents need API keys to authenticate with Cyberpunk Company. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Cyberpunk Company
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Cyberpunk Company generates a token and passes it directly to the agent
...
`CYBERPUNK_API_URL`, `CYBERPUNK_AGENT_ID`, `CYBERPUNK_COMPANY_ID`. We need to
add a `CYBERPUNK_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Cyberpunk Company CLI
...
```
cyberpunk-company auth login
# Opens browser -> user confirms -> token stored at ~/.cyberpunk-company/credentials
```
...
managed by the Cyberpunk Company server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Cyberpunk Company.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Cyberpunk Company SKILL.md (or a link to it)
   - What information Cyberpunk Company needs from the agent (e.g. webhook URL, adapter
...
5. Cyberpunk Company stores the pending registration.
...
7. On approval, Cyberpunk Company provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Cyberpunk Company issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.cyberpunk-company.ing/skills/cyberpunk-company/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Cyberpunk Company should send you heartbeats",
...
1. Generate a Cyberpunk Company invite link for the company.
...
4. A Cyberpunk Company company member approves the new agent.
5. Cyberpunk Company begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `CYBERPUNK_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `cyberpunk-company auth login` for developer-managed remote agents.                                      |
