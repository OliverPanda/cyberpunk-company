...
Today release smoke testing for published Cyberpunk Company packages is manual and shell-driven:
...
- Docker
- `npx cyberpunk-company@canary`
- `npx cyberpunk-company@latest`
...
- runs `cyberpunk-company@${CYBERPUNK COMPANYAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@cyberpunk-company.local`
- password: `cyberpunk-company-smoke-password`
...
- the suite passes locally against both:
  - `CYBERPUNK COMPANYAI_VERSION=canary`
  - `CYBERPUNK COMPANYAI_VERSION=latest`
...
1. A published `cyberpunk-company@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `cyberpunk-company@latest` Docker install can be smoke-tested by Playwright in CI.
