...
`PAP-447` asks how Cyberpunk Company should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Cyberpunk Company instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Cyberpunk Company:

- some operators use Cyberpunk Company to manage Cyberpunk Company and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Cyberpunk Company
- Claude and Codex expose different built-in affordances, so Cyberpunk Company should not overfit to one tool

## Core Product Decision

Cyberpunk Company should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Cyberpunk Company
...
- Cyberpunk Company should not assume that every adapter can see or use a host filesystem path directly
...
If Cyberpunk Company makes worktrees agent-first, it will blur:
...
- Cyberpunk Company core resolves and records execution workspace state
...
That gives Cyberpunk Company room to support:
...
- Cyberpunk Company handles workspace runtime services under the hood using project/workspace policy
...
Once Cyberpunk Company is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Cyberpunk Company should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Cyberpunk Company cannot infer
...
- when issue moves to done, should Cyberpunk Company auto-commit?
- should it auto-push?
...
Cyberpunk Company already has the right foundation for a project-first model.
...
For server-spawned local adapters, Cyberpunk Company already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Cyberpunk Company should mirror directly

Implication:

- for `codex_local`, Cyberpunk Company should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Cyberpunk Company should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Cyberpunk Company should separate:
...
Cyberpunk Company should be able to express intentions such as:
...
These are not specific to Cyberpunk Company. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".cyberpunk-company/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Cyberpunk Company-specific `devServer` flag.
...
Cyberpunk Company should distinguish between:
...
Cyberpunk Company should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
### Cyberpunk Company Core

Cyberpunk Company core should:
...
Cyberpunk Company core should not:
...
In addition to the local git helper, Cyberpunk Company should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Cyberpunk Company
...
For runtime services specifically, Cyberpunk Company should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Cyberpunk Company-only dev-server flags
...
- same embedded DB / Cyberpunk Company instance can be reused through env/config injection when appropriate
...
- Cyberpunk Company can decide whether to reuse or start a fresh service deterministically
...
3. Test against Cyberpunk Company-on-Cyberpunk Company workflows first.
...
7. Workspace runtime services are modeled generically, not as Cyberpunk Company-specific dev-server toggles.
