...
This document defines the next-stage plan for Cyberpunk Company company import/export.

The core shift is:

- move from a Cyberpunk Company-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Cyberpunk Company.
...
Cyberpunk Company already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Cyberpunk Company
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `cyberpunk-company.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.cyberpunk-company.yaml` sidecar for high-fidelity Cyberpunk Company-specific details
...
11. adapter-aware skill sync surfaces so Cyberpunk Company can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Cyberpunk Company at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Cyberpunk Company sidecar.
...
Cyberpunk Company must not redefine `SKILL.md`.
...
- Cyberpunk Company-specific fidelity lives in `.cyberpunk-company.yaml`
- Cyberpunk Company may resolve and install `SKILL.md` packages, but it must not require a Cyberpunk Company-only skill format
...
### 5.4 Base Package Vs Cyberpunk Company Extension
...
- base package:
  ...
  - no Cyberpunk Company-only runtime fields by default
- Cyberpunk Company extension:
  - `.cyberpunk-company.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Cyberpunk Company tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`cyberpunk-company.manifest.json` is not part of the future package direction.
...
Cyberpunk Company import/export should support these entity kinds:
...
In Cyberpunk Company V2 portability:
...
- Cyberpunk Company can treat “this set of agents came from team package X” as the imported-team model
...
Cyberpunk Company should support source references in package metadata with:
...
Cyberpunk Company should define a new adapter surface area around skills:
...
Planned Cyberpunk Company behavior:

- if an adapter supports read, Cyberpunk Company should show current skills in the UI
- if an adapter supports write, Cyberpunk Company should let the user enable/disable imported skills
- if an adapter supports sync, Cyberpunk Company should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.cyberpunk-company.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.cyberpunk-company.yaml` alongside the base package
...
## 10. Storage Model Inside Cyberpunk Company
...
Cyberpunk Company should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `cyberpunk-company company export <company-id> --out <path>`
- `cyberpunk-company company import <path-or-url> --dry-run`
- `cyberpunk-company company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Cyberpunk Company storage, or only referenced at import time?
...
3. Should Cyberpunk Company support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Cyberpunk Company aligned with:
