...
- local mode that still feels instant (`npx cyberpunk-company run` and go)
...
2. CLI command `pnpm cyberpunk-company auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm cyberpunk-company onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm cyberpunk-company onboard`).
...
- expose key instance settings in API + CLI (`cyberpunk-company configure` / `cyberpunk-company onboard`)
...
7. `pnpm cyberpunk-company onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
