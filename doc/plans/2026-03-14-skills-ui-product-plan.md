...
This document defines the product and UI plan for skill management in Cyberpunk Company.
...
- Cyberpunk Company company import/export can include skills as package content
...
3. The UI must always tell the truth about what Cyberpunk Company knows:
   - desired state in Cyberpunk Company
...
Cyberpunk Company should treat skills at two scopes:
...
- created locally inside Cyberpunk Company later
...
7. “Understand what Cyberpunk Company knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Cyberpunk Company-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Cyberpunk Company did not install.

These should be clearly marked:

- external
- not managed by Cyberpunk Company
...
- `desired_only`: Cyberpunk Company wants it, adapter does not show it yet
- `external`: adapter has it but Cyberpunk Company does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Cyberpunk Company only tracks desired state
...
- Cyberpunk Company can still track desired skills
...
- Cyberpunk Company can see adapter skills
...
- `.cyberpunk-company.yaml` may add Cyberpunk Company-specific fidelity, but should not replace the base shortname association model
...
- `.cyberpunk-company.yaml` may add Cyberpunk Company-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Cyberpunk Company should have a company-scoped skill model or managed package model representing:
...
Cyberpunk Company should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Cyberpunk Company-specific.
...
That gives Cyberpunk Company one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
