# Cyberpunk Company Memory Service Plan

## Goal

Define a Cyberpunk Company memory service and surface API that can sit above multiple memory backends, while preserving Cyberpunk Company's control-plane requirements:

- company scoping
- auditability
- provenance back to Cyberpunk Company work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current Cyberpunk Company architecture in:
...
Cyberpunk Company should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing Cyberpunk Company plugin runtime.

That keeps the core small and matches the current direction that optional knowledge-like systems live at the edges.

### 4. Cyberpunk Company owns routing, provenance, and accounting

Providers should not decide how Cyberpunk Company entities map to governance.

Cyberpunk Company core should own:
...
The normalized Cyberpunk Company scope passed into a provider request.
...
A normalized write, query, browse, or delete action performed through Cyberpunk Company.

Cyberpunk Company should log every operation, whether the provider is local or external.
...
## What Cyberpunk Company Should Persist

Cyberpunk Company should not mirror the full provider memory corpus into Postgres unless the provider is a Cyberpunk Company-managed local provider.

Cyberpunk Company core should persist:
...
1. `pre-run hydrate`
   Before an agent run starts, Cyberpunk Company may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, Cyberpunk Company may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, Cyberpunk Company may capture selected issue comments or issue documents as memory sources.
...
Cyberpunk Company should give agents both automatic recall and explicit tools, with simple guidance:
...
Cyberpunk Company needs a first-class UI for memory, otherwise providers become black boxes.
...
Cyberpunk Company should roll up:
...
This is important because a memory system that "works" but silently burns budget is not acceptable in Cyberpunk Company.
...
- it matches Cyberpunk Company's local-first posture
- it is inspectable
...
- Do we want Cyberpunk Company-managed extraction pipelines at all, or should built-ins be the only place where Cyberpunk Company owns extraction?
...
- Cyberpunk Company owns memory bindings, scopes, provenance, governance, and usage reporting.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives Cyberpunk Company a stable "memory service" without locking the product to one memory philosophy or one vendor.
