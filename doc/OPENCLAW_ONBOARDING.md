...
1. Start Cyberpunk Company in auth mode.
```bash
cd <cyberpunk-company-repo-root>
...
3. In Cyberpunk Company UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Cyberpunk Company UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Cyberpunk Company invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Cyberpunk Company with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Cyberpunk Company UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.cyberpunk-company-openclaw.override.yml logs -f openclaw-gateway
```
