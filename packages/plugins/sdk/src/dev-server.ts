 ...
 *
 * Endpoint summary:
 * - `GET /__cyberpunk-company__/health` for diagnostics
 * - `GET /__cyberpunk-company__/events` for hot-reload stream
...
export async function startPluginDevServer(options: PluginDevServerOptions = {}): Promise<PluginDevServer> {
  ...
  const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (url === "/__cyberpunk-company__/health") {
      sendJson(res, { ok: true, rootDir, uiDir });
      return;
    }

    if (url === "/__cyberpunk-company__/events") {
      res.writeHead(200, {
