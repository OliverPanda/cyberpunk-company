...
type GlobalBridge = typeof globalThis & {
  __cyberpunkPluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__cyberpunkPluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `Cyberpunk Company plugin UI runtime is not initialized for "${name}". ` +
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`Cyberpunk Company plugin UI component "${name}" is not callable`);
}
