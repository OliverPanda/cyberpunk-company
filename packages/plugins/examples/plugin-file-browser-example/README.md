...
Example Cyberpunk Company plugin that demonstrates:
...
```bash
pnpm --filter @cyberpunk-company/plugin-file-browser-example build
pnpm cyberpunk-company plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm cyberpunk-company plugin uninstall cyberpunk-company-file-browser-example --force
```
...
- Optional: use `cyberpunk-company-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
