...
import type { Goal, Issue } from "@cyberpunk-company/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the Cyberpunk Company APIs.",
...
const plugin: CyberpunkPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
