...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".cyberpunk-company-sdk");
...
/**
 * Generate a complete Cyberpunk Company plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Cyberpunk Company plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "cyberpunk-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["cyberpunk-company", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@cyberpunk-company/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@cyberpunk-company/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@cyberpunk-company/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@cyberpunk-company/plugin-sdk\` and \`@cyberpunk-company/shared\` from a local Cyberpunk Company checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.cyberpunk-company-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-cyberpunk-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <cyberpunk-company-sdk-path>]");
    process.exit(1);
  }
