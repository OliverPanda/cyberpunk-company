...
function resolveCyberpunkInstanceId(): string {
  const raw = process.env.CYBERPUNK_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveCyberpunkHomeDir(), "instances", resolveCyberpunkInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveCyberpunkHomeDir(), "instances", resolveCyberpunkInstanceId(), "data", "backups");
}
