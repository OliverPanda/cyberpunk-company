...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "cyberpunk-company-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveCyberpunkHomeDir(), "instances", resolveCyberpunkInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".cyberpunk-company", CONFIG_BASENAME);
...
function resolveCyberpunkConfigPath(): string {
  if (process.env.CYBERPUNK_CONFIG?.trim()) {
    return path.resolve(process.env.CYBERPUNK_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveCyberpunkEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveCyberpunkEnvPath(configPath);
