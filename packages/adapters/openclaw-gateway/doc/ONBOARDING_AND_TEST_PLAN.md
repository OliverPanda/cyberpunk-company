...
## Scope
This plan is now **gateway-only**. Cyberpunk Company supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Cyberpunk Company skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Cyberpunk Company task.
...
```bash
pnpm --filter @cyberpunk-company/server test:run -- openclaw-gateway
pnpm --filter @cyberpunk-company/server typecheck
pnpm --filter @cyberpunk-company/ui typecheck
pnpm --filter cyberpunk-company typecheck
```
