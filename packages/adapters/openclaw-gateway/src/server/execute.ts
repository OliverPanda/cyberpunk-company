...
const DEFAULT_CLIENT_VERSION = "cyberpunk-company";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "cyberpunk-company";
  if (input.strategy === "run") return `cyberpunk-company:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `cyberpunk-company:issue:${input.issueId}`;
  return fallback;
}
...
function resolveCyberpunkApiUrlOverride(value: unknown): string | null {
...
function buildWakeText(payload: WakePayload, cyberpunkCompanyEnv: Record<string, string>): string {
  ...
  const orderedKeys = [
    "CYBERPUNK_RUN_ID",
    "CYBERPUNK_AGENT_ID",
    "CYBERPUNK_COMPANY_ID",
    "CYBERPUNK_API_URL",
    "CYBERPUNK_TASK_ID",
    "CYBERPUNK_WAKE_REASON",
    "CYBERPUNK_WAKE_COMMENT_ID",
  ...
  const lines = [
    ...
    ...envLines,
    `CYBERPUNK_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load CYBERPUNK_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $CYBERPUNK_API_KEY on every API call.",
    "- Use X-Cyberpunk Company-Run-Id: $CYBERPUNK_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: CYBERPUNK_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$CYBERPUNK_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$CYBERPUNK_COMPANY_ID/issues?assigneeAgentId=$CYBERPUNK_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardCyberpunkCompanyPayload(
  ...
  const runtimeServiceIntents = Array.isArray(ctx.context.cyberpunkRuntimeServiceIntents)
    ? ctx.context.cyberpunkRuntimeServiceIntents.filter(
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "cyberpunk-company-complete");
    this.ws = null;
  }
