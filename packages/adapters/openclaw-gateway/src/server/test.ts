...
import { asString, parseObject } from "@cyberpunk-company/adapter-utils/server-utils";
        ...
        ws.send(
          JSON.stringify({
            ...
            params: {
              ...
              client: {
                id: "gateway-client",
                version: "cyberpunk-company-probe",
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Cyberpunk Company server host.",
        });
      }
    } catch (err) {
      checks.push({
