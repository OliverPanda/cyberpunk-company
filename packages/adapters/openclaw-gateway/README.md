...
This document describes how `@cyberpunk-company/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Cyberpunk Company logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Cyberpunk Company `runId`)
