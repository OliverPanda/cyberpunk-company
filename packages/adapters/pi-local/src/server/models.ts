...
import type { AdapterModel } from "@cyberpunk-company/adapter-utils";
import { asString, runChildProcess } from "@cyberpunk-company/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.CYBERPUNK_PI_COMMAND === "string" &&
    process.env.CYBERPUNK_PI_COMMAND.trim().length > 0
      ? process.env.CYBERPUNK_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["CYBERPUNK_", "npm_", "NPM_"] as const;
