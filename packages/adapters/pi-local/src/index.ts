...
Use when:
- You want Cyberpunk Company to run Pi (the AI coding agent) locally as the agent runtime
...
- Cyberpunk Company requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/cyberpunk-company/ and resumed with --session.
