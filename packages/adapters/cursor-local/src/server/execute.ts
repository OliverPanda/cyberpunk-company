...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries = options.skillsEntries
    ?? (options.skillsDir
      ...
      : await readCyberpunkCompanyRuntimeSkillEntries({}, __moduleDir));
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[cyberpunk-company] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[cyberpunk-company] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[cyberpunk-company] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[cyberpunk-company] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const cursorSkillEntries = await readCyberpunkCompanyRuntimeSkillEntries(config, __moduleDir);
  const desiredCursorSkillNames = resolveCyberpunkDesiredSkillNames(config, cursorSkillEntries);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[cyberpunk-company] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: cyberpunkCompanyEnvNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[cyberpunk-company] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
