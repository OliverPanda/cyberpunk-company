...
Use when:
- You want Cyberpunk Company to run Cursor Agent CLI locally as the agent runtime
...
- Cyberpunk Company auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$cyberpunk-company" and related skills on local runs.
- Cyberpunk Company auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
