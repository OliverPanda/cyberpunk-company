...
```sh
curl -sS -X POST "$CYBERPUNK_API_URL/api/companies/$CYBERPUNK_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$CYBERPUNK_API_URL/api/companies/$CYBERPUNK_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $CYBERPUNK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$CYBERPUNK_API_URL/api/companies/$CYBERPUNK_COMPANY_ID/skills" \
  -H "Authorization: Bearer $CYBERPUNK_API_KEY"
```
...
```sh
curl -sS "$CYBERPUNK_API_URL/api/companies/$CYBERPUNK_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $CYBERPUNK_API_KEY"

curl -sS "$CYBERPUNK_API_URL/api/companies/$CYBERPUNK_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $CYBERPUNK_API_KEY"
```
...
```sh
curl -sS -X POST "$CYBERPUNK_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $CYBERPUNK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$CYBERPUNK_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $CYBERPUNK_API_KEY"
```
...
```sh
curl -sS -X POST "$CYBERPUNK_API_URL/api/companies/$CYBERPUNK_COMPANY_ID/agents" \
  -H "Authorization: Bearer $CYBERPUNK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Cyberpunk Company runtime skills are still added automatically when required by the adapter.
