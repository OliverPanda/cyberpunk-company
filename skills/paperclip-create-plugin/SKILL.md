---
name: cyberpunk-company-create-plugin
description: >
  Create new Cyberpunk Company plugins with the current alpha SDK/runtime. Use when
...
# Create a Cyberpunk Company Plugin

Use this skill when the task is to create, scaffold, or document a Cyberpunk Company plugin.
...
node packages/plugins/create-cyberpunk-company-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Cyberpunk Company repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.cyberpunk-company-sdk/`:

```bash
pnpm --filter @cyberpunk-company/create-cyberpunk-company-plugin build
node packages/plugins/create-cyberpunk-company-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Cyberpunk Company from an absolute local path during development
