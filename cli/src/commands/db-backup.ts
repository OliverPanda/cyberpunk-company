...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@cyberpunk-company/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://cyberpunk:cyberpunk@127.0.0.1:${port}/cyberpunk-company`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  const defaultDir = resolveDefaultBackupDir(resolveCyberpunkInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "cyberpunk-company";
