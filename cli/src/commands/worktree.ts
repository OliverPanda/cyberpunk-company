...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.CYBERPUNK_CONFIG;
...
const WORKTREE_NAME_PREFIX = "cyberpunk-company-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.CYBERPUNK_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.CYBERPUNK_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromCyberpunkCompanyConfig(config: CyberpunkCompanyConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromCyberpunkCompanyConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.cyberpunk-company${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".cyberpunk-company", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".cyberpunk-company", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.cyberpunk-company"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: CyberpunkCompanyConfig, envEntries: Record<string, string>, portOverride?: number): string {
...
export function copySeededSecretsKey(input: {
  ...
  sourceConfig: CyberpunkCompanyConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.CYBERPUNK_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.CYBERPUNK_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.CYBERPUNK_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.CYBERPUNK_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: CyberpunkCompanyConfig;
  targetConfig: CyberpunkCompanyConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveCyberpunkEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readCyberpunkCompanyEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://cyberpunk:cyberpunk@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "cyberpunk-company");
    const targetConnectionString = `postgres://cyberpunk:cyberpunk@127.0.0.1:${targetHandle.port}/cyberpunk-company`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readCyberpunkCompanyEnvEntries(resolveCyberpunkEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.CYBERPUNK_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.CYBERPUNK_AGENT_JWT_SECRET);
  mergeCyberpunkCompanyEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { CYBERPUNK_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadCyberpunkCompanyEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Cyberpunk Company inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" cyberpunk-company worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printCyberpunkCompanyCliBanner();
  p.intro(pc.bgCyan(pc.black(" cyberpunk-company worktree:make ")));
...
type MergeSourceChoice = {
  ...
  branch: string | null;
  branchLabel: string;
  hasCyberpunkCompanyConfig: boolean;
  isCurrent: boolean;
};
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasCyberpunkCompanyConfig: existsSync(path.resolve(worktreePath, ".cyberpunk-company", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printCyberpunkCompanyCliBanner();
  p.intro(pc.bgCyan(pc.black(" cyberpunk-company worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveCyberpunkEnvFile(configPath);
  const envEntries = readCyberpunkCompanyEnvEntries(envPath);
  const out = {
    CYBERPUNK_CONFIG: configPath,
    ...(envEntries.CYBERPUNK_HOME ? { CYBERPUNK_HOME: envEntries.CYBERPUNK_HOME } : {}),
    ...(envEntries.CYBERPUNK_INSTANCE_ID ? { CYBERPUNK_INSTANCE_ID: envEntries.CYBERPUNK_INSTANCE_ID } : {}),
    ...(envEntries.CYBERPUNK_CONTEXT ? { CYBERPUNK_CONTEXT: envEntries.CYBERPUNK_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasCyberpunkCompanyConfig)
      .map((choice) => path.resolve(choice.worktree, ".cyberpunk-company", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readCyberpunkCompanyEnvEntries(resolveCyberpunkEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Cyberpunk Company once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasCyberpunkCompanyConfig ? "cyberpunk-company" : "no-cyberpunk-company-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".cyberpunk-company", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".cyberpunk-company", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .cyberpunk-company/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasCyberpunkCompanyConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Cyberpunk Company worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasCyberpunkCompanyConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Cyberpunk Company worktrees were found. Run `cyberpunk-company worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Cyberpunk Company configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Cyberpunk Company instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Cyberpunk Company instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with cyberpunk-company- if needed (created at ~/cyberpunk-company-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: CYBERPUNK_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: CYBERPUNK_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source CYBERPUNK_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Cyberpunk Company instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Cyberpunk Company worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with cyberpunk-company- if needed")
