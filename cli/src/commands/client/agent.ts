...
import type { Agent } from "@cyberpunk-company/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveCyberpunkSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export CYBERPUNK_API_URL='${escaped(input.apiBase)}'`,
    `export CYBERPUNK_COMPANY_ID='${escaped(input.companyId)}'`,
    `export CYBERPUNK_AGENT_ID='${escaped(input.agentId)}'`,
    `export CYBERPUNK_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Cyberpunk Company skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Cyberpunk Company skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveCyberpunkSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Cyberpunk Company skills directory. Expected ./skills in the repo checkout.",
              );
            }
