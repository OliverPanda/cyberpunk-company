...
import { ApiRequestError, CyberpunkCompanyApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Cyberpunk Company config file")
    .option("-d, --data-dir <path>", "Cyberpunk Company data directory root (isolates state from ~/.cyberpunk-company)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Cyberpunk Company API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.CYBERPUNK_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.CYBERPUNK_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.CYBERPUNK_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set CYBERPUNK_COMPANY_ID, or set context profile companyId via `cyberpunk-company context set`.",
    );
  }

  const api = new CyberpunkCompanyApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.CYBERPUNK_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.CYBERPUNK_SERVER_PORT || "");
