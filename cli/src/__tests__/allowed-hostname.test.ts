...
function createTempConfigPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-company-allowed-hostname-"));
  return path.join(dir, "config.json");
}

function writeBaseConfig(configPath: string) {
  const base: CyberpunkCompanyConfig = {
    $meta: {
      version: 1,
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/cyberpunk-company-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/cyberpunk-company-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/cyberpunk-company-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/cyberpunk-company-storage" },
      s3: {
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/cyberpunk-company-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as CyberpunkCompanyConfig;
