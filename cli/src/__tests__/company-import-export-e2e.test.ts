...
function createServerEnv(configPath: string, port: number, connectionString: string) {
  ...
  env.CYBERPUNK_CONFIG = configPath;
  env.DATABASE_URL = connectionString;
  env.HOST = "127.0.0.1";
  env.PORT = String(port);
  env.SERVE_UI = "false";
  env.CYBERPUNK_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.CYBERPUNK_MIGRATION_AUTO_APPLY = "true";
  env.CYBERPUNK_UI_DEV_MIDDLEWARE = "false";
...
function createCliEnv() {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.CYBERPUNK_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.CYBERPUNK_MIGRATION_AUTO_APPLY;
  delete env.CYBERPUNK_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(args: string[], opts: { apiBase: string; configPath: string }) {
  ...
  const result = await execFileAsync(
    "pnpm",
    ["--silent", "cyberpunk-company", ...args, "--api-base", opts.apiBase, "--config", opts.configPath, "--json"],
    {
      cwd: repoRoot,
      env: createCliEnv(),
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `cyberpunk-company run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("cyberpunk-company company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "cyberpunk-company-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");

    tempDb = await startEmbeddedPostgresTestDatabase("cyberpunk-company-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["cyberpunk-company", "run", "--config", configPath],
      {
        cwd: repoRoot,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    expect(readFileSync(path.join(exportDir, ".cyberpunk-company.yaml"), "utf8")).toContain('schema: "cyberpunk-company/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "cyberpunk-company-demo"));
