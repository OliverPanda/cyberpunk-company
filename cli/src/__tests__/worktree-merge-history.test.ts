...
describe("worktree merge history planner", () => {
  ...
  it("plans selected project imports and preserves project workspace links", () => {
    const sourceProject = makeProject({
      id: "source-project-1",
      name: "Cyberpunk Company Evals",
      goalId: "goal-1",
    });
    const sourceWorkspace = makeProjectWorkspace({
      id: "source-workspace-1",
      projectId: "source-project-1",
      cwd: "/Users/dotta/cyberpunk-company-evals",
      repoUrl: "https://github.com/cyberpunk-company/cyberpunk-company-evals.git",
    });
    ...
    expect(plan.projectImports[0]).toMatchObject({
      source: { id: "source-project-1", name: "Cyberpunk Company Evals" },
      targetGoalId: "goal-1",
    ...
    expect(insert.mappedProjectName).toBe("Cyberpunk Company Evals");
    expect(insert.adjustments).toEqual([]);
  });
