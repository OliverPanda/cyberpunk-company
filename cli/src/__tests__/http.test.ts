...
import { ApiConnectionError, ApiRequestError, CyberpunkCompanyApiClient } from "../client/http.js";

describe("CyberpunkCompanyApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new CyberpunkCompanyApiClient({
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new CyberpunkCompanyApiClient({ apiBase: "http://localhost:3100" });
