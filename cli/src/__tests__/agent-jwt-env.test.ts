...
import {
  ...
  readCyberpunkCompanyEnvEntries,
...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-company-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  ...
  it("writes .env next to explicit config path", () => {
    ...
    expect(contents).toContain("CYBERPUNK_AGENT_JWT_SECRET=");
  });

  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "CYBERPUNK_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.CYBERPUNK_AGENT_JWT_SECRET).toBe("test-secret");
  });
  ...
  it("quotes hash-prefixed env values so dotenv round-trips them", () => {
    ...
    mergeCyberpunkCompanyEnvEntries(
      {
        CYBERPUNK_WORKTREE_COLOR: "#439edb",
      },
      envPath,
    );

    const contents = fs.readFileSync(envPath, "utf-8");
    expect(contents).toContain('CYBERPUNK_WORKTREE_COLOR="#439edb"');
    expect(readCyberpunkCompanyEnvEntries(envPath).CYBERPUNK_WORKTREE_COLOR).toBe("#439edb");
  });
});
