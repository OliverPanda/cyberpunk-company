...
describe("applyDataDirOverride", () => {
  ...
  it("sets CYBERPUNK_HOME and isolated default config/context paths", () => {
    ...
    expect(process.env.CYBERPUNK_CONTEXT).toBe(path.resolve(expectedHome, "context.json"));
    expect(process.env.CYBERPUNK_INSTANCE_ID).toBe("default");
  });
  ...
  it("does not override explicit config/context settings", () => {
    process.env.CYBERPUNK_CONFIG = "/env/config.json";
    ...
    expect(process.env.CYBERPUNK_CONFIG).toBe("/env/config.json");
    expect(process.env.CYBERPUNK_CONTEXT).toBe("/env/context.json");
  });
