...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("cyberpunk-company-pr-432")).toBe(
      path.resolve(os.homedir(), "cyberpunk-company-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("cyberpunk-company/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://cyberpunk-company.example", 3110)).toBe("https://cyberpunk-company.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/cyberpunk-company-feature",
      homeDir: "/tmp/cyberpunk-company-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/cyberpunk-company-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/cyberpunk-company-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.CYBERPUNK_HOME).toBe(path.resolve("/tmp/cyberpunk-company-worktrees"));
    expect(env.CYBERPUNK_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.CYBERPUNK_IN_WORKTREE).toBe("true");
    expect(env.CYBERPUNK_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.CYBERPUNK_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export CYBERPUNK_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-worktree-secrets-"));
    const originalInlineMasterKey = process.env.CYBERPUNK_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.CYBERPUNK_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.CYBERPUNK_SECRETS_MASTER_KEY;
      delete process.env.CYBERPUNK_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.CYBERPUNK_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.CYBERPUNK_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.CYBERPUNK_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          CYBERPUNK_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.CYBERPUNK_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.CYBERPUNK_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".cyberpunk-company-worktrees"),
      });

      const envPath = path.join(repoRoot, ".cyberpunk-company", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("CYBERPUNK_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("CYBERPUNK_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/CYBERPUNK_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.CYBERPUNK_AGENT_JWT_SECRET;
      } else {
        process.env.CYBERPUNK_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".cyberpunk-company-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".cyberpunk-company", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Cyberpunk Company config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/cyberpunk-company",
        targetRepoRoot: "/Users/example/cyberpunk-company-pr-432",
        workspaceCwd: "/Users/example/cyberpunk-company",
      }),
    ).toBe("/Users/example/cyberpunk-company-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/cyberpunk-company",
        targetRepoRoot: "/Users/example/cyberpunk-company-pr-432",
        workspaceCwd: "/Users/example/cyberpunk-company/packages/db",
      }),
    ).toBe("/Users/example/cyberpunk-company-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyberpunk-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "cyberpunk-company-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("cyberpunk-company-make-test", {
        seed: false,
        home: path.join(tempRoot, ".cyberpunk-company-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".cyberpunk-company", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".cyberpunk-company", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
