...
import { cyberpunkCompanyConfigSchema, type CyberpunkCompanyConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".cyberpunk-company", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveCyberpunkInstanceId());
}
...
export function readConfig(configPath?: string): CyberpunkCompanyConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = cyberpunkCompanyConfigSchema.safeParse(migrated);
  if (!parsed.success) {
