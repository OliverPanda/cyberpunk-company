...
export class CyberpunkCompanyApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-cyberpunk-company-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the Cyberpunk Company server is not running, the configured URL is wrong, or the request is being blocked before it reaches Cyberpunk Company.",
    "",
    "Try:",
    "- Start Cyberpunk Company with `pnpm dev` or `pnpm cyberpunk-company run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Cyberpunk Company is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`CYBERPUNK_API_URL\`.`,
  );
  return lines.join("\n");
}
