...
export function resolveBoardAuthStorePath(overridePath?: string): string {
  ...
  if (process.env.CYBERPUNK_AUTH_STORE?.trim()) return path.resolve(process.env.CYBERPUNK_AUTH_STORE.trim());
...
export async function loginBoardCli(params: {
  ...
  const challenge = await requestJson<CreateChallengeResponse>(createUrl, {
    ...
    body: JSON.stringify({
      command,
      clientName: params.clientName?.trim() || "cyberpunk-company cli",
