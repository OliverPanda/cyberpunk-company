...
export function configCheck(configPath?: string): CheckResult {
  ...
  if (!configExists(configPath)) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `cyberpunk-company onboard` to create one",
    };
  }
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Run `cyberpunk-company configure --section database` (or `cyberpunk-company onboard` to recreate)",
    };
  }
}
