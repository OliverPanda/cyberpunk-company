...
export async function databaseCheck(config: CyberpunkCompanyConfig, configPath?: string): Promise<CheckResult> {
  if (config.database.mode === "postgres") {
    if (!config.database.connectionString) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `cyberpunk-company configure --section database`",
      };
    }
