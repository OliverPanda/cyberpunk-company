...
const DATA_DIR_OPTION_HELP =
  "Cyberpunk Company data directory root (isolates state from ~/.cyberpunk-company)";

program
  .name("cyberpunk-company")
  .description("Cyberpunk Company CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadCyberpunkCompanyEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start Cyberpunk Company immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Cyberpunk Company setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "cyberpunk-company")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Cyberpunk Company")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Cyberpunk Company server API")
