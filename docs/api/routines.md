...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Cyberpunk Company-Signature` + `X-Cyberpunk Company-Timestamp` header pair matching the trigger's signing mode.
