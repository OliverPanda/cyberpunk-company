...
Cyberpunk Company companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .cyberpunk-company.yaml     # Adapter config, env inputs, routines
```
...
- **.cyberpunk-company.yaml** holds Cyberpunk Company-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
cyberpunk-company company export <company-id> --out ./my-export
```
...
cyberpunk-company company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
cyberpunk-company company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
cyberpunk-company company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.cyberpunk-company.yaml`
...
```sh
# From a local folder
cyberpunk-company company import ./my-export

# From a GitHub URL
cyberpunk-company company import https://github.com/org/repo

# From a GitHub subfolder
cyberpunk-company company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
cyberpunk-company company import org/repo
cyberpunk-company company import org/repo/companies/acme
```
...
If `--target` is not specified, Cyberpunk Company infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
cyberpunk-company company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
cyberpunk-company company import org/company-templates/engineering-team \
  --target new \
...
```sh
cyberpunk-company company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
cyberpunk-company company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
cyberpunk-company company import ./package \
  --target new \
  --yes \
  --json
```
...
Cyberpunk Company supports several GitHub URL formats:
