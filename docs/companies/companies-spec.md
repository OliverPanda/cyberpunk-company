...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Cyberpunk Company.
...
- be useful outside Cyberpunk Company
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.cyberpunk-company.yaml`
...
Example Cyberpunk Company extension:
...
- Cyberpunk Company must not require extra top-level fields for skill validity
- Cyberpunk Company-specific extensions must live under `metadata.cyberpunk-company` or `metadata.sources`
...
For Cyberpunk Company, the preferred fidelity extension is:
...
- issue/task Cyberpunk Company-only metadata
...
- Cyberpunk Company tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Cyberpunk Company shape:
...
Additional rules for Cyberpunk Company exporters:
...
- omit empty and default-valued Cyberpunk Company fields when possible
...
- Cyberpunk Company exporters should emit `.cyberpunk-company.yaml` as a sidecar by default
...
## 19. Cyberpunk Company Mapping

Cyberpunk Company can map this spec to its runtime model like this:
...
- Cyberpunk Company extension:
  - `.cyberpunk-company.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Cyberpunk Company-specific fidelity

Inline Cyberpunk Company-only metadata that must live inside a shared markdown file should use:

- `metadata.cyberpunk-company`

That keeps the base format broader than Cyberpunk Company.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Cyberpunk Company.

## 20. Cutover

Cyberpunk Company should cut over to this markdown-first package model as the primary portability format.

`cyberpunk-company.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Cyberpunk Company, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
