# ClipHub: Marketplace for Cyberpunk Company Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Cyberpunk Company configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Cyberpunk Company-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Cyberpunk Company company (agents, projects, governance) |
...
A complete Cyberpunk Company company configuration:
...
Single-agent configurations designed to plug into a Cyberpunk Company org:
...
Portable skill files that any Cyberpunk Company agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Cyberpunk Company's skill loading system
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Cyberpunk Company company
...
| `POST` | `/api/listings/:id/install` | Install to Cyberpunk Company company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Cyberpunk Company company (or create new)
  → ClipHub API calls Cyberpunk Company API to:
  ...
  → Redirect to Cyberpunk Company dashboard with new team running
```
...
### 5.3 Creator: Export from Cyberpunk Company → Publish

```
Running Cyberpunk Company company → "Export as Blueprint" (CLI or UI)
  → Cyberpunk Company exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Cyberpunk Company brand blue/purple)
...
| Install flow | One-click deploy to Cyberpunk Company company |
...
{
  "targetCompanyId": "uuid",         // Existing Cyberpunk Company company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `cyberpunk-company-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Cyberpunk Company
- **Backend**: Node.js API (or extend Cyberpunk Company server)
- **Database**: Postgres (can share Cyberpunk Company's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Cyberpunk Company auth (or OAuth2)

### 9.2 Integration with Cyberpunk Company

ClipHub can be:
- **Option A**: A separate app that calls Cyberpunk Company's API to install blueprints
- **Option B**: A built-in section of the Cyberpunk Company UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Cyberpunk Company UI and API.
...
- [ ] Install flow: blueprint → Cyberpunk Company company
...
- [ ] "Export from Cyberpunk Company" CLI command
