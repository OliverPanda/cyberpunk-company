...
Cyberpunk Company supports two runtime modes with different security profiles.
...
```sh
pnpm cyberpunk-company allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Cyberpunk Company emits a one-time claim URL at startup:
...
```sh
CYBERPUNK_DEPLOYMENT_MODE=authenticated pnpm cyberpunk-company run
```
