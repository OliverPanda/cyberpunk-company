...
Build a custom adapter to connect Cyberpunk Company to any agent runtime.
...
2. Build environment with `buildCyberpunk CompanyEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `cyberpunk-company run --watch` using `picocolors`.
...
Make Cyberpunk Company skills discoverable to your agent runtime without writing to the agent's working directory:
