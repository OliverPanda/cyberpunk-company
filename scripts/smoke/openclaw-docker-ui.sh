...
CYBERPUNK_HOST_PORT="${CYBERPUNK_HOST_PORT:-3100}"
CYBERPUNK_HOST_FROM_CONTAINER="${CYBERPUNK_HOST_FROM_CONTAINER:-host.docker.internal}"
...
detect_cyberpunk-company_base_url() {
  ...
  for candidate in "$CYBERPUNK_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${CYBERPUNK_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${CYBERPUNK_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
cyberpunk-company_base_url="$(detect_cyberpunk-company_base_url || true)"
...
EOF
  if [[ -n "$cyberpunk-company_base_url" ]]; then
    cat <<EOF
  $cyberpunk-company_base_url
