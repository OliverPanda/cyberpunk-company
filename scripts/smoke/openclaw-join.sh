...
COMPANY_ID="${COMPANY_ID:-${CYBERPUNK_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-cyberpunk-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-cyberpunk-openclaw-smoke}"
...
Provide one of:
  CYBERPUNK_AUTH_HEADER=\"Bearer <board-token>\"
  CYBERPUNK_COOKIE=\"<board-session-cookie>\"
...
log "checking Cyberpunk Company health"
...
assert_status "200"
if ! grep -q "Cyberpunk Company OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.cyberpunk.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
