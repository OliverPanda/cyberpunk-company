...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set CYBERPUNK_API_KEY=${AGENT_API_KEY} in your run context before running Paperclip heartbeat steps."
...
main() {
  ...
  wait_http_ready "${CYBERPUNK_API_URL%/}/api/health" 15 || fail "Paperclip API health endpoint not reachable"
