...
CYBERPUNK_API_URL="${CYBERPUNK_API_URL:-http://127.0.0.1:3100}"
API_BASE="${CYBERPUNK_API_URL%/}/api"
...
CYBERPUNK_API_URL_FOR_OPENCLAW="${CYBERPUNK_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${CYBERPUNK_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${CYBERPUNK_AUTH_HEADER}" )
fi
if [[ -n "${CYBERPUNK_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${CYBERPUNK_COOKIE}" )
  CYBERPUNK_BROWSER_ORIGIN="${CYBERPUNK_BROWSER_ORIGIN:-${CYBERPUNK_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${CYBERPUNK_BROWSER_ORIGIN}" -H "Referer: ${CYBERPUNK_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set CYBERPUNK_COOKIE or CYBERPUNK_AUTH_HEADER."
  fi
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/cyberpunk-company"
  local claimed_file="${workspace_dir}/cyberpunk-company-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/cyberpunk-company-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/cyberpunk-company-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/cyberpunk-company-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/cyberpunk-company"
    assert_status "200"
    {
      echo "CYBERPUNK_API_URL=${CYBERPUNK_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set CYBERPUNK_API_KEY=${AGENT_API_KEY} in your run context before running Cyberpunk Company heartbeat steps."
...
main() {
  ...
  wait_http_ready "${CYBERPUNK_API_URL%/}/api/health" 15 || fail "Cyberpunk Company API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "cyberpunk-company health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
