...
CYBERPUNK_DEPLOYMENT_MODE="${CYBERPUNK_DEPLOYMENT_MODE:-authenticated}"
CYBERPUNK_DEPLOYMENT_EXPOSURE="${CYBERPUNK_DEPLOYMENT_EXPOSURE:-private}"
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$CYBERPUNK_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$CYBERPUNK_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$CYBERPUNK_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$CYBERPUNK_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$CYBERPUNK_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$CYBERPUNK_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $CYBERPUNK_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $CYBERPUNK_DEPLOYMENT_MODE/$CYBERPUNK_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$CYBERPUNK_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $CYBERPUNK_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$CYBERPUNK_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $CYBERPUNK_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
