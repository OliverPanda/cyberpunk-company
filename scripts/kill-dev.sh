#!/usr/bin/env bash
#
# Kill all local Cyberpunk Company dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all cyberpunk-company dev processes
...
# Collect PIDs of node processes running from any cyberpunk-company directory.
# Matches paths like /Users/*/cyberpunk-company/... or /Users/*/cyberpunk-company-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/cyberpunk-company(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No Cyberpunk Company dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} Cyberpunk Company dev process(es):"
echo ""
