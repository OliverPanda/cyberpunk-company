...
source_config_path="${CYBERPUNK_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.cyberpunk-company/config.json" || -L "$base_cwd/.cyberpunk-company/config.json" ) ]]; then
...
async function main() {
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.CYBERPUNK_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.CYBERPUNK_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.CYBERPUNK_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "CYBERPUNK_HOME=" + JSON.stringify(worktreeHome),
    "CYBERPUNK_INSTANCE_ID=" + JSON.stringify(instanceId),
    "CYBERPUNK_CONFIG=" + JSON.stringify(configPath),
    "CYBERPUNK_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "CYBERPUNK_IN_WORKTREE=true",
    "CYBERPUNK_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.CYBERPUNK_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("CYBERPUNK_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
