#!/bin/bash -ex
export PC_TEST_ROOT="$(mktemp -d /tmp/cyberpunk-company-clean.XXXXXX)"
export PC_HOME="$PC_TEST_ROOT/home"
export PC_CACHE="$PC_TEST_ROOT/npm-cache"
export PC_DATA="$PC_TEST_ROOT/cyberpunk-company-data"
mkdir -p "$PC_HOME" "$PC_CACHE" "$PC_DATA"
echo "PC_TEST_ROOT: $PC_TEST_ROOT"
echo "PC_HOME: $PC_HOME"
cd $PC_TEST_ROOT
git clone https://github.com/cyberpunk-company/cyberpunk-company.git repo
cd repo
pnpm install
