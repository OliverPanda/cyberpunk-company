...
# This keeps @cyberpunk-company/server publish artifacts self-contained for static UI serving.

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
UI_DIST="$REPO_ROOT/ui/dist"
SERVER_UI_DIST="$REPO_ROOT/server/ui-dist"

echo "  -> Building @cyberpunk-company/ui..."
pnpm --dir "$REPO_ROOT" --filter @cyberpunk-company/ui build
