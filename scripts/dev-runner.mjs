...
const ignoredRelativePaths = new Set([
  ".cyberpunk-company/dev-server-status.json",
]);
...
if (mode === "watch") {
  env.CYBERPUNK_MIGRATION_PROMPT ??= "never";
  env.CYBERPUNK_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.CYBERPUNK_DEPLOYMENT_MODE = "authenticated";
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.CYBERPUNK_MIGRATION_AUTO_APPLY === "true";
