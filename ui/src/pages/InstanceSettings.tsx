...
import type { InstanceSchedulerHeartbeatAgent } from "@cyberpunk-company/shared";
...
export function InstanceSettings() {
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? `停用 1 个定时心跳失败：${detail}`
            : `共 ${enabled.length} 个定时心跳，其中 ${failures.length} 个停用失败。首个错误：${detail}`,
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "停用全部心跳失败。");
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> 个启用</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> 个停用</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> 家公司</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(`确定要停用全部 ${enabledCount} 个已启用的定时心跳吗？`)) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? "停用中..." : "全部停用"}
          </Button>
        )}
      </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? "停用定时心跳" : "启用定时心跳"}
                          </Button>
                        </span>
                      </div>
                    );
