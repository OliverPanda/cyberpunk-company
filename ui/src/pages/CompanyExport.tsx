...
const ROLE_LABELS: Record<string, string> = {
  ...
  vp: "VP", manager: "经理", engineer: "工程师", agent: "智能体",
};
...
function generateReadmeFromSelection(
  ...
  if (agents.length > 0) counts.push(["智能体", agents.length]);
...
export function CompanyExport() {
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: "导出已下载",
        body: `已导出 ${resultCheckedFiles.size} 个文件，下载包名为 ${result.rootPath}.zip`,
      });
    },
    onError: (err) => {
      pushToast({
  ...
  const effectiveFiles = useMemo(() => {
    ...
    if (typeof exportData.files["README.md"] === "string") {
      const companyName = exportData.manifest.company?.name ?? selectedCompany?.name ?? "公司";
