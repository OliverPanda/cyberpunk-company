...
function renderImportFileExtra(node: FileTreeNode, checked: boolean, renameMap: Map<string, string>) {
  ...
  const actionBadge = node.action ? (
    <span className={cn(
      ...
      {checked ? actionLabel(node.action) : "跳过"}
    </span>
  ) : null;
...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message="请选择一个文件以预览其内容。" />
    );
  }
            ...
            return (
              ...
              >
                ...
                <span className={cn(
                  ...
                  {item.kind === "agent" ? "智能体" : item.kind === "project" ? "项目" : item.kind === "issue" ? "任务" : "技能"}
                </span>
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (Object.keys(archive.files).length === 0) {
    throw new Error("所选 zip 压缩包中未找到任何包文件。");
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error("尚未配置导入来源。");
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message="请选择要导入到哪家公司。" />;
  }
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label="GitHub 地址"
            hint="仓库目录路径，或指向 COMPANY.md 的 blob 地址（例如 github.com/owner/repo/tree/main/company）。"
          >
            <input
        ...
        <Field label="目标" hint="导入到当前公司，或新建一家公司。">
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? "预览中..." : "预览导入内容"}
          </Button>
        </div>
      </div>
