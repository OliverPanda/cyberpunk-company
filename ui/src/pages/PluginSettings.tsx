...
export function PluginSettings() {
  ...
  const displayStatus = formatPluginLifecycleStatus(plugin.status);
  const statusVariant =
    plugin.status === "ready"
      ? "default"
      : plugin.status === "error"
                          ...
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              ...
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {formatPluginLifecycleStatus(dashboardData.worker.status)}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                            ...
                            {dashboardData.worker.totalCrashes > 0 && (
                              <>
                                ...
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">最近一次崩溃</span>
                    ...
                    <div className="max-h-64 space-y-1 overflow-y-auto font-mono text-xs">
                      {recentLogs.map((entry) => (
                        ...
                        >
                          ...
                          <Badge variant="outline" className="h-4 shrink-0 px-1 text-[10px]">{formatLogLevel(entry.level)}</Badge>
                    ...
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={healthData.healthy ? "default" : "destructive"}>
                          {formatPluginLifecycleStatus(healthData.status)}
                        </Badge>
                      </div>
...
function formatPluginLifecycleStatus(status: string | null | undefined): string {
  switch (status) {
    case "ready":
      return "就绪";
    case "running":
