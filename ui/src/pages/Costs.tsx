...
function budgetScopeTypeLabel(scopeType: "company" | "agent" | "project") {
  if (scopeType === "company") return "公司";
  if (scopeType === "agent") return "智能体";
  return "项目";
}
    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={`当前区间共 ${eventCount} 条事件`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label="入账"
          value={formatCents(creditCents)}
          subtitle="退款、抵扣和返还信用额度"
          icon={ArrowDownLeft}
        />
        <MetricTile
          label="净额"
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="请选择一个公司以查看成本。" />;
  }
      ...
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  推理支出、平台费用、返还信用额度，以及实时 quota 窗口。
                </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            ...
            <MetricTile
              ...
              subtitle={`当前区间内预估 ${formatCents(financeData?.summary.estimatedDebitCents ?? 0)}`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          当前区间已消耗月度预算的 {spendData.summary.utilizationPercent}%。
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>每个智能体在所选时间段内的消耗情况。</CardDescription>
                  </CardHeader>
                              ...
                              <div className="text-right text-sm tabular-nums">
                                  ...
                                  输入 {formatTokens(row.inputTokens + row.cachedInputTokens)} · 输出 {formatTokens(row.outputTokens)}
                                    ...
                                    {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                      <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0 ? `${row.apiRunCount} 个 API` : "0 个 API"}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? `${row.subscriptionRunCount} 个订阅`
                                      : "0 个订阅"}
                                      </div>
                                    ) : null}
                              </div>
                ...
                <div className="space-y-4">
                  <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">按项目统计</CardTitle>
                    <CardDescription>通过项目关联 issue 归因的运行成本。</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">暂无已归因到项目的运行成本。</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="暂无财务事件。等账单或返还信用额度到账后，再补充账户级费用。" />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  ...
                  <CardDescription>
                    面向智能体和项目的硬停止支出上限。提供商订阅配额单独计算，并显示在“提供商”页签下。
                  </CardDescription>
                </CardHeader>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      你可以在这里通过提高预算，或显式保持暂停状态，来处理硬停止事件。
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">{budgetScopeTypeLabel(scopeType)}预算</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "公司级月度策略。"
                            : scopeType === "agent"
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      还没有预算策略。可以在智能体和项目详情页设置预算，或使用现有的公司月度预算控制。
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">当前时间范围内没有成本事件。</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">当前时间范围内没有可计费事件。</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>按收费或入账主体分组的账户级财务事件。</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">暂时还没有财务事件。</p>
                      ) : (
