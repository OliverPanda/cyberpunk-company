...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "如果已有运行正在执行，则只保留一个后续待运行任务。",
  always_enqueue: "每次触发都进入队列，即使该例程当前已经在运行。",
  skip_if_active: "当该例程仍在运行时，忽略新的触发事件。",
};
...
function concurrencyPolicyLabel(value: string) {
  switch (value) {
...
function catchUpPolicyLabel(value: string) {
  switch (value) {
    case "skip_missed":
      return "跳过错过的触发";
...
function routineRunStatusLabel(value: string) {
  switch (value) {
    case "queued":
      return "排队中";
    case "running":
            ...
            <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
              <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">策略控制应服务于工作定义本身。</p>
                </div>
      ...
      <div>
        {(routines ?? []).length === 0 ? (
          <div className="py-12">
            <EmptyState
              icon={Repeat}
              message="还没有例程。使用“新建例程”来定义第一个周期性工作流。"
            />
          </div>
        ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-muted-foreground">
                        ...
                        {routine.lastRun ? (
                          <div className="mt-1 text-xs">{routineRunStatusLabel(routine.lastRun.status)}</div>
                        ) : null}
                      </td>
