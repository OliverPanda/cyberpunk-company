...
const surfaceOptions: Array<{
...
}> = [
  {
    ...
    label: "运行详情",
    eyebrow: "完整转录",
    description: "完整运行页面，带有“美化 | 原始”切换，并提供最便于检查的转录视图。",
    icon: MonitorCog,
  },
  {
    id: "live",
    label: "事项挂件",
    eyebrow: "实时流",
    description: "事项详情页中的实时运行挂件，针对不离开任务页面跟进活跃运行进行了优化。",
    icon: RadioTower,
  },
  {
    id: "dashboard",
    label: "仪表盘卡片",
    eyebrow: "紧凑卡片",
    description: "活跃智能体仪表盘卡片，在保持相同转录语言的同时优化了紧凑浏览体验。",
    icon: PanelsTopLeft,
  },
];
            ...
            <div className="mb-5 flex flex-wrap items-center gap-2">
              ...
              <div className="inline-flex rounded-full border border-border/70 bg-background/80 p-1">
                {([
                  ["comfortable", "舒适"],
                  ["compact", "紧凑"],
                ] as const).map(([nextDensity, label]) => (
                  <button
                    key={nextDensity}
                    type="button"
                    className={cn(
