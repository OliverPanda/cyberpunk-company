...
import type { Approval, HeartbeatRun, Issue, JoinRequest } from "@cyberpunk-company/shared";
import {
...
export function Inbox() {
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : "批准加入申请失败");
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : "拒绝加入申请失败");
    },
  });
  ...
  const retryRunMutation = useMutation({
    mutationFn: async (run: HeartbeatRun) => {
      ...
      if (!("id" in result)) {
        throw new Error("已跳过重试，因为当前无法唤醒该智能体。");
      }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canMarkAllRead && (
            ...
            >
              {markAllReadMutation.isPending ? "标记中…" : "全部标记为已读"}
            </Button>
          )}
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:justify-end">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="issues_i_touched">我最近处理的事项</SelectItem>
                <SelectItem value="join_requests">加入申请</SelectItem>
                ...
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      ...
                      {dashboard!.agents.error === 1 ? "个智能体出现错误" : "个智能体出现错误"}
                    </span>
                  </Link>
                  <button
                    type="button"
