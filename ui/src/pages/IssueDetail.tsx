...
import type { ActivityEvent } from "@cyberpunk-company/shared";
import type { Agent, IssueAttachment } from "@cyberpunk-company/shared";
...
function documentDisplayLabel(key: string) {
  return key === "document" ? "文档" : key;
}

function humanizeValue(value: unknown): string {
  if (typeof value !== "string") return String(value ?? "无");
  if (value in ISSUE_STATUS_LABELS) return ISSUE_STATUS_LABELS[value];
  if (value in ISSUE_PRIORITY_LABELS) return ISSUE_PRIORITY_LABELS[value];
...
function formatAction(action: string, details?: Record<string, unknown> | null): string {
  ...
  if (
    ...
    return `${ACTION_LABELS[action] ?? action} ${documentDisplayLabel(key)}${title}`;
  }
...
export function IssueDetail() {
  ...
  const importMarkdownDocument = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : "导入文档失败");
    },
  });
  ...
  const attachmentUploadButton = (
    <>
      ...
      >
        <Cyberpunk Company className="h-3.5 w-3.5 mr-1.5" />
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        ...
        <TabsContent value="comments">
          <CommentThread
            ...
            agentMap={agentMap}
            draftKey={`cyberpunk-company:issue-comment-draft:${issue.id}`}
            enableReassign
                      ...
                      {(() => {
                        switch (approval.type) {
                          ...
                          case "approve_ceo_strategy":
                            return "CEO 策略";
                          default:
                            return String(approval.type).replace(/_/g, " ");
                        }
                      })()}
                    </span>
