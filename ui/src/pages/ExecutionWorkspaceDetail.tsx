  ...
  return (
    <div className="max-w-2xl space-y-4">
      ...
      <div className="rounded-lg border border-border p-4">
        ...
        <DetailRow label="最近使用">{new Date(workspace.lastUsedAt).toLocaleString()}</DetailRow>
        <DetailRow label="清理">
          {workspace.cleanupEligibleAt ? `${new Date(workspace.cleanupEligibleAt).toLocaleString()}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}` : "未安排"}
        </DetailRow>
      </div>
    </div>
  );
