...
import type { ApprovalComment } from "@cyberpunk-company/shared";
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">发起人</span>
              <Identity
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              请在 <Link to="/costs" className="underline underline-offset-2">/costs</Link> 的预算控制中解除此预算停用。
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("要删除这个未获批准的智能体吗？此操作无法撤销。")) return;
