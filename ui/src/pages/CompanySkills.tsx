...
function formatProjectScanSummary(result: CompanySkillProjectScanResult) {
  const parts = [
    `${result.discovered} 个已发现`,
    `${result.imported.length} 个已导入`,
    `${result.updated.length} 个已更新`,
  ];
  if (result.conflicts.length > 0) parts.push(`${result.conflicts.length} 个冲突`);
  if (result.skipped.length > 0) parts.push(`${result.skipped.length} 个已跳过`);
  return `扫描了 ${result.scannedWorkspaces} 个工作区，${parts.join("，")}。`;
}
    ...
    <div className="min-w-0">
      <div className="border-b border-border px-5 py-4">
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          ...
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            ...
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">没有关联智能体</span>
            ) : (
      ...
      <div className="min-h-[560px] px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">选择一个文件以查看内容。</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
