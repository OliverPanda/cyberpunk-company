...
const DRAFT_KEY = "cyberpunk:issue-draft";
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          ...
          body: `${failures.length} 个暂存${failures.length === 1 ? "文件" : "文件"}未能添加。`,
          tone: "warn",
          action: prefix
      ...
      >
          ...
          <div className="px-4 py-3 shrink-0 space-y-2">
            <div className="space-y-1.5">
              ...
              <div className="text-[11px] text-muted-foreground">
                控制该任务是在共享工作区、新建隔离工作区，还是现有工作区中运行。
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  正在复用 {selectedReusableExecutionWorkspace.name}，来源于 {selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? "现有执行工作区"}。
                </div>
              )}
            </div>
          </div>
                  ...
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <Cyberpunk Company className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                            <span className="truncate text-sm">{file.file.name}</span>
                          </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap shrink-0">
          ...
          <button
            ...
            disabled={createIssue.isPending}
          >
            <Cyberpunk Company className="h-3 w-3" />
            上传
          </button>
