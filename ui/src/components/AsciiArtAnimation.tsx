...
type CyberpunkSprite = (typeof CYBERPUNK_SPRITES)[number];
...
function spriteSize(sprite: CyberpunkSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = CYBERPUNK_SPRITES[(baseCol + baseRow) % CYBERPUNK_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = CYBERPUNK_SPRITES[Math.floor(Math.random() * CYBERPUNK_SPRITES.length)]!;
