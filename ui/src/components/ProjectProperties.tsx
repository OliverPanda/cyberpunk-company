...
function ProjectStatusPicker({ status, onChange }: { status: string; onChange: (status: string) => void }) {
  ...
  const currentLabel = PROJECT_STATUSES.find((item) => item.value === status)?.label ?? status.replace("_", " ");
  ...
  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? "归档后，该项目会从侧边栏和项目选择器中隐藏。"
          : "取消归档后，该项目会重新出现在侧边栏和项目选择器中。"}
      </p>
      {archivePending ? (
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    {codebase.localFolder ? "修改本地文件夹" : "设置本地文件夹"}
                  </Button>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          主机托管实现：<span className="text-foreground">Git 工作树</span>
                        </div>
                        <div>
                      ...
                      <span>工作树父目录</span>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          准备命令会在派生出的工作树内、智能体执行前运行。清理命令会保存在这里，供后续清理流程使用。
                        </p>
                      </div>
                    ) : null}
                  </div>
