import type { FinanceEvent } from "@cyberpunk-company/shared";
...
export function FinanceTimelineCard({
  rows,
  emptyMessage = "当前时间范围内没有财务事件。",
}: FinanceTimelineCardProps) {
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">近期财务事件</CardTitle>
        <CardDescription>充值、手续费、入账、预留费用及其他非单次请求费用。</CardDescription>
      </CardHeader>
                ...
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>账单 {row.externalInvoiceId}</div> : null}
