...
import type { BudgetPolicySummary } from "@cyberpunk-company/shared";
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>剩余</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : "不限额"}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? "这个项目的执行已暂停，直到预算被提高或该事件被处理。"
          : "这个作用域的心跳已暂停，直到预算被提高或该事件被处理。"}
      </div>
    </div>
  ) : null;
