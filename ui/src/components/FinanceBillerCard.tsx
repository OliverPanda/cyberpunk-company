import type { FinanceByBiller } from "@cyberpunk-company/shared";
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1 text-xs">
              {row.eventCount} 条事件，涉及 {row.kindCount} 类费用
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-3 px-4 pb-4 pt-3">
        <div className="grid gap-2 text-sm sm:grid-cols-3">
          <div className="border border-border p-3">
            <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">支出</div>
            <div className="mt-1 font-medium tabular-nums">{formatCents(row.debitCents)}</div>
          </div>
          <div className="border border-border p-3">
            <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">入账</div>
