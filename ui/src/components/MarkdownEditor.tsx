...
import { buildAgentMentionHref, buildProjectMentionHref } from "@cyberpunk-company/shared";
...
export const MarkdownEditor = forwardRef<MarkdownEditorRef, MarkdownEditorProps>(function MarkdownEditor({
  ...
  const plugins = useMemo<RealmPlugin[]>(() => {
    const imageHandler = hasImageUpload
      ? async (file: File) => {
          ...
          if (!handler) throw new Error("缺少图片上传处理器");
          try {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative cyberpunk-company-mdxeditor-scope",
    ...
    >
      <MDXEditor
        ...
        onBlur={() => onBlur?.()}
        className={cn("cyberpunk-company-mdxeditor", !bordered && "cyberpunk-company-mdxeditor--borderless")}
        contentEditableClassName={cn(
          "cyberpunk-company-mdxeditor-content focus:outline-none [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:list-item",
          contentClassName,
        )}
