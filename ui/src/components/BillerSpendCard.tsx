...
import type { CostByBiller, CostByProviderModel } from "@cyberpunk-company/shared";
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {budgetMonthlyCents > 0 && (
          <QuotaBar
            ...
            rightLabel={`占配额 ${Math.round(budgetPct)}%`}
          />
        )}

        <div className="text-xs text-muted-foreground">
          {row.apiRunCount > 0 ? `${row.apiRunCount} 次按量计费运行` : "0 次按量计费运行"}
          {" · "}
