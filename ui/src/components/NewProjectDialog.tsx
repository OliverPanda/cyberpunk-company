...
import { PROJECT_COLORS } from "@cyberpunk-company/shared";
...
export function NewProjectDialog() {
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error("尚未选择公司");
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError("本地文件夹必须填写完整的绝对路径。");
      return;
    }
    if (repoUrl && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError("仓库必须填写有效的 GitHub 仓库 URL。");
      return;
    }
            ...
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <Tooltip delayDuration={300}>
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  关联一个 GitHub 仓库后，智能体就能为该项目克隆、读取并推送代码。
                </TooltipContent>
              </Tooltip>
            </div>
            <input
