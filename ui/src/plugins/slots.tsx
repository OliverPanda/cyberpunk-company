 ...
 *
 * Plugin bundles are built with `external: ["@cyberpunk-company/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__cyberpunkPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__cyberpunkPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__cyberpunkPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__cyberpunkPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@cyberpunk-company/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@cyberpunk-company/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@cyberpunk-company/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@cyberpunk-company/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@cyberpunk-company/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__cyberpunkPluginBridge__) {
 ...
 * self-register via `window.cyberpunkPlugins.registerReactComponent()`.
 ...
 * of `@cyberpunk-company/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
