...
import { parseAgentMentionHref, parseProjectMentionHref } from "@cyberpunk-company/shared";
...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--cyberpunk-company-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--cyberpunk-company-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("cyberpunk-company-mention-chip", `cyberpunk-company-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("cyberpunk-company-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    "cyberpunk-company-mention-chip",
    "cyberpunk-company-mention-chip--agent",
    "cyberpunk-company-mention-chip--project",
    "cyberpunk-company-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--cyberpunk-company-mention-project-color");
  element.style.removeProperty("--cyberpunk-company-mention-icon-mask");
}
