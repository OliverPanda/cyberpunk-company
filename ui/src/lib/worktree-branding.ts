...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("cyberpunk-worktree-enabled") !== "true") return null;

  const name = readMetaContent("cyberpunk-worktree-name");
  const color = normalizeHexColor(readMetaContent("cyberpunk-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("cyberpunk-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
