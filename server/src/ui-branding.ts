const FAVICON_BLOCK_START = "<!-- CYBERPUNK_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- CYBERPUNK_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- CYBERPUNK_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- CYBERPUNK_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.CYBERPUNK_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.CYBERPUNK_WORKTREE_NAME) ?? nonEmpty(env.CYBERPUNK_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.CYBERPUNK_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="cyberpunk-worktree-enabled" content="true" />',
    `<meta name="cyberpunk-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="cyberpunk-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="cyberpunk-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
