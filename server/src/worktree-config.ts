...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Cyberpunk Company worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.CYBERPUNK_IN_WORKTREE !== "true") return null;

  const configPath = resolveCyberpunkConfigPath(overrideConfigPath);
  const envPath = resolveCyberpunkEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.CYBERPUNK_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.CYBERPUNK_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.CYBERPUNK_HOME) ??
      nonEmpty(env.CYBERPUNK_WORKTREES_DIR) ??
      "~/.cyberpunk-company-worktrees",
  );
...
function writeConfigFile(configPath: string, config: CyberpunkCompanyConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".cyberpunk-company", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as CyberpunkCompanyConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: CyberpunkCompanyConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: CyberpunkCompanyConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.CYBERPUNK_HOME = context.homeDir;
  process.env.CYBERPUNK_INSTANCE_ID = context.instanceId;
  process.env.CYBERPUNK_CONFIG = context.configPath;
  process.env.CYBERPUNK_CONTEXT = context.contextPath;
  process.env.CYBERPUNK_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as CyberpunkCompanyConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    CYBERPUNK_HOME: context.homeDir,
    CYBERPUNK_INSTANCE_ID: context.instanceId,
    CYBERPUNK_CONFIG: context.configPath,
    CYBERPUNK_CONTEXT: context.contextPath,
    CYBERPUNK_IN_WORKTREE: "true",
    CYBERPUNK_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: CyberpunkCompanyConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as CyberpunkCompanyConfig;
  } catch {
    return;
  }
