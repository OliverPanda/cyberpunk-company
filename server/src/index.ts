...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.CYBERPUNK_SECRETS_PROVIDER === undefined) {
    process.env.CYBERPUNK_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.CYBERPUNK_SECRETS_STRICT_MODE === undefined) {
    process.env.CYBERPUNK_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.CYBERPUNK_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.CYBERPUNK_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.CYBERPUNK_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.CYBERPUNK_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set CYBERPUNK_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@cyberpunk-company.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.CYBERPUNK_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.CYBERPUNK_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or CYBERPUNK_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.CYBERPUNK_LISTEN_HOST = runtimeListenHost;
  process.env.CYBERPUNK_LISTEN_PORT = String(listenPort);
  process.env.CYBERPUNK_API_URL = `http://${runtimeApiHost}:${listenPort}`;
    ...
    const runScheduledBackup = async () => {
      ...
      try {
        const result = await runDatabaseBackup({
          ...
          filenamePrefix: "cyberpunk-company",
        });
        logger.info(
          {
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.CYBERPUNK_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.CYBERPUNK_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Cyberpunk Company server failed to start");
    process.exit(1);
  });
}
