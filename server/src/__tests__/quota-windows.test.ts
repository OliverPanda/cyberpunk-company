...
import type { QuotaWindow } from "@cyberpunk-company/adapter-utils";
...
describe("readClaudeToken", () => {
  ...
  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `cyberpunk-company-test-claude-${Date.now()}`);
...
describe("readCodexAuthInfo", () => {
  ...
  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `cyberpunk-company-test-codex-${Date.now()}`);
    await import("node:fs/promises").then((fs) =>
      fs.mkdir(tmpDir, { recursive: true }).then(() =>
