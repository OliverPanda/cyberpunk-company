...
describe("agent skill routes", () => {
  beforeEach(() => {
    ...
    mockCompanySkillService.listRuntimeSkillEntries.mockResolvedValue([
      {
        key: "cyberpunk-company/cyberpunk-company/cyberpunk-company",
        runtimeName: "cyberpunk-company",
        source: "/tmp/cyberpunk-company",
        required: true,
    ...
    mockCompanySkillService.resolveRequestedSkillKeys.mockImplementation(
      async (_companyId: string, requested: string[]) =>
        requested.map((value) =>
          value === "cyberpunk-company"
            ? "cyberpunk-company/cyberpunk-company/cyberpunk-company"
            : value,
        ),
    );
    mockAdapter.listSkills.mockResolvedValue({
      ...
      supported: true,
      mode: "ephemeral",
      desiredSkills: ["cyberpunk-company/cyberpunk-company/cyberpunk-company"],
      entries: [],
      warnings: [],
    });
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await request(createApp())
      ...
      .send({ desiredSkills: ["cyberpunk-company/cyberpunk-company/cyberpunk-company"] });
  ...
  it("canonicalizes desired skill references before syncing", async () => {
    ...
    const res = await request(createApp())
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["cyberpunk-company"] });

    expect(res.status, JSON.stringify(res.body)).toBe(200);
    expect(mockCompanySkillService.resolveRequestedSkillKeys).toHaveBeenCalledWith("company-1", ["cyberpunk-company"]);
    expect(mockAgentService.update).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          cyberpunkSkillSync: expect.objectContaining({
            desiredSkills: ["cyberpunk-company/cyberpunk-company/cyberpunk-company"],
          }),
        }),
      }),
