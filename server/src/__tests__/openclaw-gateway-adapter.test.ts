...
import { execute, testEnvironment } from "@cyberpunk-company/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@cyberpunk-company/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@cyberpunk-company/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              cyberpunkRuntimeServiceIntents: [
                {
                  name: "preview",
                  lifecycle: "ephemeral",
                },
      ...
      expect(payload?.sessionKey).toBe("cyberpunk:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("CYBERPUNK_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("CYBERPUNK_TASK_ID=task-123");
