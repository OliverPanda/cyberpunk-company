...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    ...
    await fs.writeFile(
      envPath,
      [
        "# Cyberpunk Company environment variables",
        "CYBERPUNK_IN_WORKTREE=true",
        "CYBERPUNK_WORKTREE_NAME=PAP-884-ai-commits-component",
        "CYBERPUNK_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.CYBERPUNK_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.CYBERPUNK_WORKTREES_DIR = isolatedHome;
    delete process.env.CYBERPUNK_HOME;
    delete process.env.CYBERPUNK_INSTANCE_ID;
    delete process.env.CYBERPUNK_CONFIG;
    delete process.env.CYBERPUNK_CONTEXT;
    ...
    expect(repairedEnv).toContain(`CYBERPUNK_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('CYBERPUNK_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`CYBERPUNK_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`CYBERPUNK_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('CYBERPUNK_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.CYBERPUNK_HOME).toBe(isolatedHome);
    expect(process.env.CYBERPUNK_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });
  ...
  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "# Cyberpunk Company environment variables",
        "CYBERPUNK_IN_WORKTREE=true",
        "CYBERPUNK_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.CYBERPUNK_IN_WORKTREE = "true";
    process.env.CYBERPUNK_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.CYBERPUNK_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cyberpunk-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".cyberpunk-company-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".cyberpunk-company", "worktrees");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    ...
    process.chdir(worktreeRoot);
    process.env.CYBERPUNK_IN_WORKTREE = "true";
    process.env.CYBERPUNK_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.CYBERPUNK_HOME = isolatedHome;
    process.env.CYBERPUNK_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
