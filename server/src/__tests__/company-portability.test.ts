...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        markdown: "---\nname: cyberpunk-company\ndescription: Cyberpunk Company coordination skill\n---\n\n# Cyberpunk Company\n",
        sourceType: "github",
        sourceLocator: "https://github.com/cyberpunk-company/cyberpunk-company/tree/master/skills/cyberpunk-company",
        ...
        metadata: {
          sourceKind: "github",
          owner: "cyberpunk-company",
          repo: "cyberpunk-company",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
          repoSkillDir: "skills/cyberpunk-company",
        },
      },
      {
        id: "skill-2",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/cyberpunk-company/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "cyberpunk-company",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/cyberpunk-company/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "cyberpunk-company",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Cyberpunk Company extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Cyberpunk Company"');
    ...
    expect(exported.files["skills/cyberpunk-company/cyberpunk-company/cyberpunk-company/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".cyberpunk-company.yaml"]);
    expect(extension).toContain('schema: "cyberpunk-company/v1"');
    ...
    expect(extension).not.toContain("cyberpunkSkillSync");
  ...
  it("exports default sidebar order into the Cyberpunk Company extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".cyberpunk-company.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/cyberpunk-company/cyberpunk-company/cyberpunk-company/SKILL.md"])).toContain("# Cyberpunk Company");
    expect(asTextFile(exported.files["skills/cyberpunk-company/cyberpunk-company/cyberpunk-company/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/cyberpunk-company/cyberpunk-company/cyberpunk-company/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/cyberpunk-company/cyberpunk-company/cyberpunk-company/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/cyberpunk-company/cyberpunk-company/cyberpunk-company/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .cyberpunk-company.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".cyberpunk-company.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      {
        id: "skill-cyberpunk-company",
        companyId: "company-1",
        key: "cyberpunk-company/cyberpunk-company/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/cyberpunk-company/cyberpunk-company/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "cyberpunk_bundled",
          owner: "cyberpunk-company",
          repo: "cyberpunk-company",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/cyberpunk-company/cyberpunk-company/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/cyberpunk-company/cyberpunk-company/release-changelog/SKILL.md"])).toContain("cyberpunk-company/cyberpunk-company/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/cyberpunk-company",
            repoUrl: "https://github.com/cyberpunk-company/cyberpunk-company.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .cyberpunk-company-tmp",
            remoteProvider: null,
          ...
          },
          {
            ...
            name: "Local Scratch",
            sourceType: "local_path",
            cwd: "/tmp/cyberpunk-company-local",
            repoUrl: null,
            repoRef: null,
            defaultRef: null,
            visibility: "advanced",
    ...
    expect(extension).toContain('repoUrl: "https://github.com/cyberpunk-company/cyberpunk-company.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/cyberpunk-company");
    ...
    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Cyberpunk Company",
    });
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Cyberpunk Company",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "cyberpunk-company-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/cyberpunk-company/cyberpunk-company.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    projectSvc.list.mockResolvedValue([
      {
        id: "project-1",
        name: "Cyberpunk Company App",
        urlKey: "cyberpunk-company-app",
        description: "Ship it",
        leadAgentId: null,
        targetDate: null,
        color: null,
    ...
    const extension = asTextFile(exported.files[".cyberpunk-company.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/cyberpunk-company/cyberpunk-company.git"');
    expect(extension).toContain('projectWorkspaceKey: "cyberpunk-company"');
  ...
  it("reads env inputs back from .cyberpunk-company.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Cyberpunk Company routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "cyberpunk-company-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Cyberpunk Company" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Cyberpunk Company"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .cyberpunk-company.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Cyberpunk Company");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".cyberpunk-company.yaml"] = `${exported.files[".cyberpunk-company.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
