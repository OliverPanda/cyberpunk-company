...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Cyberpunk Company OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Cyberpunk Company base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("cyberpunkCompanyApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.cyberpunkCompanyApiUrl");
    expect(text).toContain("~/.openclaw/workspace/cyberpunk-company-claimed-api-key.json");
    expect(text).toContain("CYBERPUNK_API_KEY");
