...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "cyberpunk-company", LOGNAME: "cyberpunk-company", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "cyberpunk-company" }),
      },
    );

    expect(tokens).toEqual(["cyberpunk-company", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "cyberpunk-company" }, {
        userInfo: () => ({ username: "cyberpunk-company" }),
      });

      expect(tokens).toEqual(["cyberpunk-company", "custom-token"]);
    } finally {
  ...
  it("reports matches without leaking which token was searched", () => {
    ...
    const exitCode = runForbiddenTokenCheck({
      repoRoot: "/repo",
      tokens: ["cyberpunk-company", "custom-token"],
      exec,
      log,
      error,
    });
