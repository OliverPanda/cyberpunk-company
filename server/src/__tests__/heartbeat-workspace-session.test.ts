...
import type { agents } from "@cyberpunk-company/db";
import { sessionCodec as codexSessionCodec } from "@cyberpunk-company/adapter-codex-local/server";
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[cyberpunk-company] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables Cyberpunk Company-managed rotation by default for codex and claude local", () => {
