...
import { companySkills } from "@cyberpunk-company/db";
import { readCyberpunkSkillSyncPreference, writeCyberpunkSkillSyncPreference } from "@cyberpunk-company/adapter-utils/server-utils";
import type { CyberpunkSkillEntry } from "@cyberpunk-company/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@cyberpunk-company/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveCyberpunkInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("cyberpunk-company/cyberpunk-company/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveCyberpunkInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "cyberpunk_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Cyberpunk Company skills are read-only.",
      sourceLabel: "Cyberpunk Company bundled",
      sourceBadge: "cyberpunk-company",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Cyberpunk Company workspace",
        sourceBadge: "cyberpunk-company",
...
export function companySkillService(db: Db) {
        ...
        if (!adapter?.listSkills) {
          ...
          try {
            ...
            const snapshot = await adapter.listSkills({
              ...
              config: {
                ...runtimeConfig,
                cyberpunkCompanyRuntimeSkills: runtimeSkillEntries,
              },
            });
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "cyberpunk_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Cyberpunk Company skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "cyberpunk_bundled"
        && incomingKind === "github"
        && incomingOwner === "cyberpunk-company"
        && incomingRepo === "cyberpunk-company"
      ) {
        out.push(existing);
        continue;
      }
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeCyberpunkSkillSyncPreference(config, filtered),
        });
      }
    }
