...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@cyberpunk-company/db";
...
import type {
  ...
  PluginWorkspace,
  IssueComment,
} from "@cyberpunk-company/plugin-sdk";
 ...
 *
 * This implementation delegates to the core Cyberpunk Company domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@cyberpunk-company/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
