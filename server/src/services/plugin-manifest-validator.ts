 ...
 */
import { pluginManifestV1Schema } from "@cyberpunk-company/shared";
import type { CyberpunkPluginManifestV1 } from "@cyberpunk-company/shared";
import { PLUGIN_API_VERSION } from "@cyberpunk-company/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): CyberpunkPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as CyberpunkPluginManifestV1,
        };
      }
