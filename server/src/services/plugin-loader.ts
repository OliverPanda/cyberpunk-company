 ...
 *    (`~/.cyberpunk-company/plugins/`) and `node_modules` for packages matching
 *    the `cyberpunk-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Cyberpunk Company plugins.
 * Packages matching this pattern are considered Cyberpunk Company plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "cyberpunk-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: CyberpunkPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.cyberpunk-company/plugins/ local directory
  | "npm"               // npm packages matching cyberpunk-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: CyberpunkPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
   ...
   * Defaults to ~/.cyberpunk-company/plugins/
   */
  ...
  /**
   * Whether to discover installed npm packages matching the cyberpunk-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "cyberpunk-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: CyberpunkPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Cyberpunk Company plugins installed as npm packages in the current
   * Node.js environment matching the "cyberpunk-plugin-*" naming convention.
   *
   ...
   * the "cyberpunkPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Cyberpunk Company plugin.
   * Throws if the package is a Cyberpunk Company plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<CyberpunkPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: CyberpunkPluginManifestV1;
    newManifest: CyberpunkPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Cyberpunk Company plugin naming convention.
 * Accepts both the "cyberpunk-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @cyberpunk-company/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "cyberpunkPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const cyberpunkPlugin = pkgJson["cyberpunkPlugin"];
  if (
    cyberpunkPlugin !== null &&
    typeof cyberpunkPlugin === "object" &&
    !Array.isArray(cyberpunkPlugin)
  ) {
    const manifestRelPath = (cyberpunkPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: CyberpunkPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumCyberpunkCompanyVersion;
}
 ...
 *   packageName: "cyberpunk-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: CyberpunkPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as CyberpunkPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Cyberpunk Company plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<CyberpunkPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Cyberpunk Company plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasCyberpunkPlugin = "cyberpunkPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasCyberpunkPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<CyberpunkPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: CyberpunkPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @cyberpunk-company/shared exports). Run those workers through
