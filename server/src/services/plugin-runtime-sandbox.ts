...
import type { CyberpunkPluginManifestV1 } from "@cyberpunk-company/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __cyberpunk_company_exports: module.exports,
      __cyberpunk_company_module: module,
      __cyberpunk_company_require: requireInSandbox,
      __cyberpunk_company_filename: realPath,
      __cyberpunk_company_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__cyberpunk_company_exports, __cyberpunk_company_module, __cyberpunk_company_require, __cyberpunk_company_filename, __cyberpunk_company_dirname)`;
