...
import { agents as agentsTable, companies, heartbeatRuns } from "@cyberpunk-company/db";
...
import { runClaudeLogin } from "@cyberpunk-company/adapter-claude-local/server";
import {
...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@cyberpunk-company/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.CYBERPUNK_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeCyberpunkSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readCyberpunkSkillSyncPreference(
