...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "cyberpunk-company" &&
    normalized !== "cyberpunk-company-create-agent" &&
    normalized !== "cyberpunk-company-create-plugin" &&
...
/** Resolve the Cyberpunk Company repo skills directory (built-in / managed skills). */
function resolveCyberpunkSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
interface AvailableSkill {
  name: string;
  description: string;
  isCyberpunkCompanyManaged: boolean;
}
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const cyberpunkCompanySkillsDir = resolveCyberpunkSkillsDir();

  // Build set of Cyberpunk Company-managed skill names
  const cyberpunkCompanySkillNames = new Set<string>();
  if (cyberpunkCompanySkillsDir) {
    try {
      for (const entry of fs.readdirSync(cyberpunkCompanySkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) cyberpunkCompanySkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isCyberpunkCompanyManaged: cyberpunkCompanySkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  cyberpunkCompanyApiUrl?: unknown;
  ...
  if (!nonEmptyTrimmedString(merged.cyberpunkCompanyApiUrl)) {
    const legacyCyberpunkCompanyApiUrl = nonEmptyTrimmedString(input.cyberpunkCompanyApiUrl);
    if (legacyCyberpunkCompanyApiUrl) merged.cyberpunkCompanyApiUrl = legacyCyberpunkCompanyApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    cyberpunkCompanyApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.cyberpunkCompanyApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawCyberpunkCompanyApiUrl =
    typeof defaults.cyberpunkCompanyApiUrl === "string"
      ? defaults.cyberpunkCompanyApiUrl.trim()
      : "";
  if (rawCyberpunkCompanyApiUrl) {
    try {
      const parsedCyberpunkCompanyApiUrl = new URL(rawCyberpunkCompanyApiUrl);
      if (
        parsedCyberpunkCompanyApiUrl.protocol !== "http:" &&
        parsedCyberpunkCompanyApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_cyberpunk-company_api_url_protocol",
          level: "warn",
          message: `cyberpunkCompanyApiUrl must use http:// or https:// (got ${parsedCyberpunkCompanyApiUrl.protocol}).`
        });
      } else {
        normalized.cyberpunkCompanyApiUrl = parsedCyberpunkCompanyApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_cyberpunk-company_api_url_configured",
          level: "info",
          message: `cyberpunkCompanyApiUrl set to ${parsedCyberpunkCompanyApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_cyberpunk-company_api_url_invalid",
        level: "warn",
        message: `Invalid cyberpunkCompanyApiUrl: ${rawCyberpunkCompanyApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Cyberpunk Company host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Cyberpunk Company is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm cyberpunk-company allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/cyberpunk-company";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/cyberpunk-company-claimed-api-key.json and load CYBERPUNK_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: cyberpunkCompanyApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Cyberpunk Company hostname is reachable and allowed via `pnpm cyberpunk-company allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Cyberpunk Company API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/cyberpunk-company/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Cyberpunk Company OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Cyberpunk Company generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "cyberpunkCompanyApiUrl": "https://cyberpunk-company-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Cyberpunk Company before key claim is allowed.
    ...
    - ~/.openclaw/workspace/cyberpunk-company-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/cyberpunk-company-claimed-api-key.json

    And set the CYBERPUNK_API_KEY and CYBERPUNK_API_URL in your environment variables as specified here:
    ...
    Then set CYBERPUNK_API_KEY and CYBERPUNK_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Cyberpunk Company skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your CYBERPUNK_API_URL to the top of your skill and note the path to your CYBERPUNK_API_URL
    ...
    ${
      ...
      "Ensure Cyberpunk Company is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Cyberpunk Company base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.cyberpunkCompanyApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm cyberpunk-company allowed-hostname <host>
      - then restart Cyberpunk Company and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@cyberpunk-company.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "cyberpunk-company", path: "/api/skills/cyberpunk-company" },
        {
        ...
        },
        {
          ...
          path: "/api/skills/cyberpunk-company-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              cyberpunkCompanyApiUrl: req.body.cyberpunkCompanyApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.cyberpunkCompanyApiUrl &&
          !persistedDefaults.cyberpunkCompanyApiUrl
        ) {
          missingPersistedFields.push("cyberpunkCompanyApiUrl");
        }
