...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@cyberpunk-company/db";
import type { DeploymentMode } from "@cyberpunk-company/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  cyberpunkUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).cyberpunkUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.cyberpunkUpgradeContext = context;
