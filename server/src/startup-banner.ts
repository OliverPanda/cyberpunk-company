...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.CYBERPUNK_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.CYBERPUNK_AGENT_JWT_SECRET === "string" ? parsed.CYBERPUNK_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm cyberpunk-company onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveCyberpunkEnvPath();
