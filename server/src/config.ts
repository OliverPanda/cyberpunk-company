...
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(CYBERPUNK_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(CYBERPUNK_ENV_FILE_PATH)
  : CWD_ENV_PATH === CYBERPUNK_ENV_FILE_PATH;
...
export function loadConfig(): Config {
  ...
  const strictModeFromEnv = process.env.CYBERPUNK_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? false);

  const providerFromEnvRaw = process.env.CYBERPUNK_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.CYBERPUNK_STORAGE_PROVIDER;
  ...
  const storageS3ForcePathStyle =
    process.env.CYBERPUNK_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.CYBERPUNK_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);

  const deploymentModeFromEnvRaw = process.env.CYBERPUNK_DEPLOYMENT_MODE;
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const deploymentExposureFromEnvRaw = process.env.CYBERPUNK_DEPLOYMENT_EXPOSURE;
  ...
  const authBaseUrlModeFromEnvRaw = process.env.CYBERPUNK_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.CYBERPUNK_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.CYBERPUNK_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.CYBERPUNK_ALLOWED_HOSTNAMES;
  ...
  const companyDeletionEnvRaw = process.env.CYBERPUNK_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.CYBERPUNK_DB_BACKUP_ENABLED !== undefined
      ? process.env.CYBERPUNK_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.CYBERPUNK_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.CYBERPUNK_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      30,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.CYBERPUNK_DB_BACKUP_DIR ??
  ...
  return {
    ...
    uiDevMiddleware: process.env.CYBERPUNK_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
