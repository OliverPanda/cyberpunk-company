...
- Cyberpunk Company issue context now has incremental APIs: `GET /api/agents/me/inbox-lite`, `GET /api/issues/:id/heartbeat-context`, and comment delta queries via `GET /api/issues/:id/comments?after=...&order=asc`.
- The `cyberpunk-company` skill now teaches agents to use those compact/incremental APIs first, while keeping full-thread fetches as a cold-start fallback.
...
- Runtime skill defaults now include `cyberpunk-company`, `para-memory-files`, and `cyberpunk-company-create-agent`. `create-agent-adapter` was moved to `.agents/skills/create-agent-adapter`.

Important follow-up finding from real-run review:

- `codex_local` currently injects Cyberpunk Company skills into the shared Codex skills home (`$CODEX_HOME/skills` or `~/.codex/skills`) rather than mounting a worktree-local skill directory.
- If a Cyberpunk Company-owned skill symlink already points at another live checkout, the adapter currently skips it instead of repointing it.
- In practice, this means a worktree can contain newer `skills/cyberpunk-company/SKILL.md` guidance while Codex still follows an older checkout's skill content.
